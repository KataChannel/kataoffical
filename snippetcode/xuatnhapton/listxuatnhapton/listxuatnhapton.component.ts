...
export class ListXuatnhaptonComponent implements OnInit {
  ...
  private _XuatnhaptonService: XuatnhaptonService = inject(XuatnhaptonService);
  ...
  Listxuatnhapton = this._XuatnhaptonService.ListXuatnhapton;
  page = this._XuatnhaptonService.page;
  pageCount = this._XuatnhaptonService.pageCount;
  total = this._XuatnhaptonService.total;
  pageSize = this._XuatnhaptonService.pageSize;
  xuatnhaptonId = this._XuatnhaptonService.xuatnhaptonId;
  ...
  constructor() {
    effect(() => {
      this.dataSource.data = this.Listxuatnhapton();
  ...
  async ngOnInit(): Promise<void> {
    this._XuatnhaptonService.listenXuatnhaptonUpdates();
    await this._XuatnhaptonService.getAllXuatnhapton(this.pageSize(),true);
  ...
  async getUpdatedCodeIds() {
    await this._XuatnhaptonService.getUpdatedCodeIds();
  }
  ...
  doFilterHederColumn(event: any, column: any): void {
    this.dataSource.filteredData = this.Listxuatnhapton().filter((v: any) => 
  ...
  ResetFilter() {
    this.ListFilter = this.Listxuatnhapton();
  }
  ...
  ApplyFilterColum(menu: any) {
    this.dataSource.data = this.Listxuatnhapton().filter((v: any) => 
  ...
  DeleteListItem(): void {
    this.EditList.forEach((item: any) => {
      this._XuatnhaptonService.DeleteXuatnhapton(item);
    });
    this.EditList = [];
  ...
  goToDetail(item: any): void {
    this.drawer.open();
    this._XuatnhaptonService.setXuatnhaptonId(item.id);
  ...
  async DoImportData(data: any) {
    ...
    const existingXuatnhapton = this._XuatnhaptonService.ListXuatnhapton();
    const existingMasp = existingXuatnhapton.map((v: any) => v.masp);
    ...
    await Promise.all(uniqueData.map(async (v: any) => {
      const existingItem = existingXuatnhapton.find((v1: any) => v1.masp === v.masp);
      if (existingItem) {
        const updatedItem = { ...existingItem, ...v };
        await this._XuatnhaptonService.updateXuatnhapton(updatedItem);
      } else {
        await this._XuatnhaptonService.CreateXuatnhapton(v);
      }
    }));
    await Promise.all(existingXuatnhapton
      .filter(sp => !uniqueData.some((item: any) => item.masp === sp.masp))
      .map(sp => this._XuatnhaptonService.updateXuatnhapton({ ...sp, isActive: false }))
    );
  ...
  onPageSizeChange(size: number, menuHienthi: any) {
    ...
    this._XuatnhaptonService.page.set(1);
    this._XuatnhaptonService.getAllXuatnhapton(size, true);
    menuHienthi.closeMenu();
  }
  onPreviousPage(){
    if (this.page() > 1) {
      this._XuatnhaptonService.page.set(this.page() - 1);
      this._XuatnhaptonService.getAllXuatnhapton(this.pageSize(), true);
    }
  }
