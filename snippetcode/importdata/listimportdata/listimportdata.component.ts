...
export class ListImportdataComponent implements OnInit {
  ...
  page = this._ImportdataService.page;
  pageCount = this._ImportdataService.pageCount;
  total = this._ImportdataService.total;
  pageSize = this._ImportdataService.pageSize;
  ...
  constructor() {
    effect(() => {
      this.dataSource.data = this.Listimportdata();
  ...
  async ngOnInit(): Promise<void> {
    this._ImportdataService.listenImportdataUpdates();
    await this._ImportdataService.getAllImportdata(this.pageSize(),true);
    this.displayedColumns = Object.keys(this.ColumnName);
    this.dataSource = new MatTableDataSource(this.Listimportdata());
    this.dataSource.sort = this.sort;
  ...
  private initializeColumns(): void {
    ...
    localStorage.setItem('ImportdataColFilter', JSON.stringify(this.FilterColumns));
  ...
  async getUpdatedCodeIds() {
    await this._ImportdataService.getUpdatedCodeIds();
  }
  ...
  doFilterHederColumn(event: any, column: any): void {
    this.dataSource.filteredData = this.Listimportdata().filter((v: any) => 
  ...
  ResetFilter() {
    this.ListFilter = this.Listimportdata();
  }
  ...
  ApplyFilterColum(menu: any) {
    this.dataSource.data = this.Listimportdata().filter((v: any) => 
  ...
  DeleteListItem(): void {
    this.EditList.forEach((item: any) => {
      this._ImportdataService.DeleteImportdata(item);
    });
    this.EditList = [];
  ...
  goToDetail(item: any): void {
    this.drawer.open();
    this._ImportdataService.setImportdataId(item.id);
    this._router.navigate(['admin/importdata', item.id]);
  }
  ...
  async DoImportData(data: any) {
    ...
    const existingImportdata = this._ImportdataService.ListImportdata();
    const existingMasp = existingImportdata.map((v: any) => v.masp);
    ...
    await Promise.all(uniqueData.map(async (v: any) => {
      const existingItem = existingImportdata.find((v1: any) => v1.masp === v.masp);
      if (existingItem) {
        const updatedItem = { ...existingItem, ...v };
        await this._ImportdataService.updateImportdata(updatedItem);
      } else {
        await this._ImportdataService.CreateImportdata(v);
      }
    }));
    await Promise.all(existingImportdata
      .filter(sp => !uniqueData.some((item: any) => item.masp === sp.masp))
      .map(sp => this._ImportdataService.updateImportdata({ ...sp, isActive: false }))
    );
  ...
  onPageSizeChange(size: number, menuHienthi: any) {
    ...
    this._ImportdataService.page.set(1);
    this._ImportdataService.getAllImportdata(size, true);
    menuHienthi.closeMenu();
  }
  onPreviousPage(){
    if (this.page() > 1) {
      this._ImportdataService.page.set(this.page() - 1);
      this._ImportdataService.getAllImportdata(this.pageSize(), true);
    }
  }
