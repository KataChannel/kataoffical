...
export class ListCongnokhachhangComponent implements OnInit {
  ...
  FilterColumns: any[] = JSON.parse(localStorage.getItem('CongnokhachhangColFilter') || '[]');
  Columns: any[] = [];
  ...
  private _CongnokhachhangService: CongnokhachhangService = inject(CongnokhachhangService);
  private _breakpointObserver: BreakpointObserver = inject(BreakpointObserver);
  private _router: Router = inject(Router);
  private _dialog: MatDialog = inject(MatDialog);
  private _snackBar: MatSnackBar = inject(MatSnackBar);

  Listcongnokhachhang = this._CongnokhachhangService.ListCongnokhachhang;
  page = this._CongnokhachhangService.page;
  totalPages = this._CongnokhachhangService.totalPages;
  total = this._CongnokhachhangService.total;
  pageSize = this._CongnokhachhangService.pageSize;
  congnokhachhangId = this._CongnokhachhangService.congnokhachhangId;
  ...
  constructor() {
    effect(() => {
      this.dataSource.data = this.Listcongnokhachhang();
  ...
  async ngOnInit(): Promise<void> {
    this._CongnokhachhangService.listenCongnokhachhangUpdates();
    await this._CongnokhachhangService.getAllCongnokhachhang(this.searchParam);
    this.displayedColumns = Object.keys(this.ColumnName);
    this.dataSource = new MatTableDataSource(this.Listcongnokhachhang());
    this.dataSource.sort = this.sort;
  ...
  private initializeColumns(): void {
    ...
    localStorage.setItem('CongnokhachhangColFilter', JSON.stringify(this.FilterColumns));
  ...
  async getUpdatedCodeIds() {
    await this._CongnokhachhangService.getUpdatedCodeIds();
  }
  ...
  DeleteListItem(): void {
    this.EditList.forEach((item: any) => {
      this._CongnokhachhangService.DeleteCongnokhachhang(item);
    });
    this.EditList = [];
  ...
  goToDetail(item: any): void {
    this.drawer.open();
    this._CongnokhachhangService.setCongnokhachhangId(item.id);
  ...
  onPageSizeChange(size: number, menuHienthi: any) {
    ...
    this._CongnokhachhangService.page.set(1);
    this._CongnokhachhangService.getAllCongnokhachhang(this.searchParam, true);
    menuHienthi.closeMenu();
  }
  onPreviousPage(){
    if (this.page() > 1) {
      this._CongnokhachhangService.page.set(this.page() - 1);
      this._CongnokhachhangService.getAllCongnokhachhang(this.searchParam, true);
    }
  }
