...
export class ListResourceComponent implements OnInit {
  ...
  FilterColumns: any[] = JSON.parse(localStorage.getItem('ResourceColFilter') || '[]');
  Columns: any[] = [];
  ...
  async getUpdatedCodeIds() {
    await this._ResourceService.getUpdatedCodeIds();
  }
  ...
  doFilterHederColumn(event: any, column: any): void {
    this.dataSource.filteredData = this.Listresource().filter((v: any) => 
  ...
  ApplyFilterColum(menu: any) {
    this.dataSource.data = this.Listresource().filter((v: any) => 
  ...
  onPageSizeChange(size: number, menuHienthi: any) {
    ...
    this._ResourceService.page.set(1);
    this._ResourceService.getAllResource(size, true);
    menuHienthi.closeMenu();
  }
  onPreviousPage(){
    if (this.page() > 1) {
      this._ResourceService.page.set(this.page() - 1);
      this._ResourceService.getAllResource(this.pageSize(), true);
    }
  }
