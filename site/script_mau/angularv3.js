...
`
    ...
    {
        originalPath: "sanpham.service.ts",
...
`
  ...
  <div class="flex flex-col space-y-2 h-screen-16 w-full p-2">
    <div *ngIf="!isSearch"
      class="p-4 cursor-pointer w-full relative flex lg:flex-row lg:space-y-2 space-y-0 flex-col space-x-2 justify-between items-center bg-white rounded-lg">
      <div class="w-full flex flex-col gap-2  lg:flex-row lg:items-center lg:justify-between">
        ...
        <div class="flex flex-row space-x-2 items-center">
          ...
          <button matTooltip="Tải file excel Mẫu" (click)="ExportExcel(Listsanpham(),'search')" color="primary"
            mat-icon-button>
    ...
    <div class="w-full h-full overflow-auto">
      <table class="overflow-hidden w-full cursor-pointer" mat-table [dataSource]="dataSource" matSort>
        @for (column of displayedColumns; track column) {
        <ng-container [matColumnDef]="column">
          @if (column == 'stt') {
          <th class="!border" mat-header-cell *matHeaderCellDef>
            <span>{{ ColumnName[column] }}</span>
          </th>
          }
          @else {
          <th class="!border whitespace-nowrap" mat-header-cell *matHeaderCellDef mat-sort-header>
        ...
        <tr class="border" mat-header-row *matHeaderRowDef="displayedColumns; sticky: true"></tr>
...
<ng-template #LoadDriveDialog>
  <mat-dialog-content
    ...
    <div class="relative h-full w-full overflow-auto">
      <app-ktable [ListItem]="ImportIteam" [ColumnName]="ImportColumnName"
        [displayedColumns]="ImportdisplayedColumns"></app-ktable>
    </div>
...
`
    },
    {
        ...
        content: `import { AfterViewInit, ChangeDetectionStrategy, Component, computed, effect, inject, TemplateRef, ViewChild } from '@angular/core';
  ...
  <ng-container *ngIf="!isDelete()">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <mat-form-field appearance="outline">
        ...
        <input matInput [(ngModel)]="DetailSanpham().status" [disabled]="!isEdit()" placeholder="Vui lòng nhập Trạng Thái"/>
      </mat-form-field>
...
`},
  ...
  export class SanphamService {
    ...
    {
        originalPath: "sanpham/sanpham.type.ts",
        content: ``
    },
    {
...
function createModuleFiles(moduleNameInput) {
    ...
    fileDefinitions.forEach(fileDef => {
        ...
        const newPathString = fileDef.originalPath.replace(/sanpham/g, lowerModuleName);
        ...
        // Sử dụng regex với global flag (g) để thay thế tất cả các instance
...
if (args.length === 0) {
    console.log("Cách sử dụng: node create_dynamic_module.js <TênModule>");
    console.log("Ví dụ: node create_dynamic_module.js product");
    process.exit(1);
}
