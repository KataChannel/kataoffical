...
export class PhieukhoService {
  ...
  async CreatePhieukho(dulieu: any) {
    try {
      ...
      this._ErrorLogService.logError('Failed to CreatePhieukho', error);
      console.error(error);
    }
  }

  async getAllPhieukho(pageSize: number = this.pageSize(), forceRefresh: boolean = false) {
    this.pageSize.set(pageSize);
    const cached = await this.getCachedData();   
    const updatedAtCache = this._StorageService.getItem('phieukhos_updatedAt') || '0';    
    
    // Nếu không yêu cầu tải mới và cache hợp lệ, trả về cache
    if (!forceRefresh && cached.phieukhos.length > 0 && Date.now() - new Date(updatedAtCache).getTime() < 5 * 60 * 1000) {
    ...
    try {
      ...
      this._ErrorLogService.logError('Failed to getAllPhieukho', error);
      console.error(error);
      this.ListPhieukho.set(cached.phieukhos);
      this.page.set(cached.pagination.page);
      this.pageCount.set(cached.pagination.pageCount);
  ...
  async getPhieukhoBy(param: any, pageSize: number = this.pageSize()) {
    ...
    try {
      ...
      this._ErrorLogService.logError('Failed to getPhieukhoBy', error);
      console.error(error);
      const cached = await this.getCachedData();
      if (!param.isOne) {
        this.ListPhieukho.set(cached.phieukhos);
  ...
  async updatePhieukho(dulieu: any) {
    try {
      ...
      this._ErrorLogService.logError('Failed to updatePhieukho', error);
      console.error(error);
    }
  }
  ...
  async DeletePhieukho(item: any) {
    try {
      ...
      this._ErrorLogService.logError('Failed to DeletePhieukho', error);
      console.error(error);
    }
  }
