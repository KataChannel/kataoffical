...
export class ListPhieukhoComponent implements OnInit {
  ...
  FilterColumns: any[] = JSON.parse(localStorage.getItem('PhieukhoColFilter') || '[]');
  Columns: any[] = [];
  ...
  page = this._PhieukhoService.page;
  pageCount = this._PhieukhoService.pageCount;
  total = this._PhieukhoService.total;
  pageSize = this._PhieukhoService.pageSize;
  ...
  constructor() {
    effect(() => {
      this.dataSource.data = this.Listphieukho();
  ...
  async ngOnInit(): Promise<void> {
    this._PhieukhoService.listenPhieukhoUpdates();
    await this._PhieukhoService.getAllPhieukho(this.pageSize(),true);
    this.displayedColumns = Object.keys(this.ColumnName);
    this.dataSource = new MatTableDataSource(this.Listphieukho());
    this.dataSource.sort = this.sort;
  ...
  async getUpdatedCodeIds() {
    await this._PhieukhoService.getUpdatedCodeIds();
  }
  ...
  doFilterHederColumn(event: any, column: any): void {
    this.dataSource.filteredData = this.Listphieukho().filter((v: any) => 
  ...
  ResetFilter() {
    this.ListFilter = this.Listphieukho();
  }
  ...
  ApplyFilterColum(menu: any) {
    this.dataSource.data = this.Listphieukho().filter((v: any) => 
  ...
  create(): void {
    this.drawer.open();
    this._router.navigate(['admin/phieukho', 'new']);
  }
  ...
  DeleteListItem(): void {
    this.EditList.forEach((item: any) => {
      this._PhieukhoService.DeletePhieukho(item);
    });
    this.EditList = [];
  ...
  async DoImportData(data: any) {
    ...
    const existingPhieukho = this._PhieukhoService.ListPhieukho();
    const existingMasp = existingPhieukho.map((v: any) => v.masp);
    ...
    await Promise.all(uniqueData.map(async (v: any) => {
      const existingItem = existingPhieukho.find((v1: any) => v1.masp === v.masp);
      if (existingItem) {
        const updatedItem = { ...existingItem, ...v };
        await this._PhieukhoService.updatePhieukho(updatedItem);
      } else {
        await this._PhieukhoService.CreatePhieukho(v);
      }
    }));
    await Promise.all(existingPhieukho
      .filter(sp => !uniqueData.some((item: any) => item.masp === sp.masp))
      .map(sp => this._PhieukhoService.updatePhieukho({ ...sp, isActive: false }))
    );
  ...
  onPageSizeChange(size: number, menuHienthi: any) {
    ...
    this._PhieukhoService.page.set(1);
    this._PhieukhoService.getAllPhieukho(size, true);
    menuHienthi.closeMenu();
  }
  onPreviousPage(){
    if (this.page() > 1) {
      this._PhieukhoService.page.set(this.page() - 1);
      this._PhieukhoService.getAllPhieukho(this.pageSize(), true);
    }
  }
