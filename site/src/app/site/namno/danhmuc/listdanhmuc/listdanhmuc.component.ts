...
@Component({
  ...
  styleUrls: ['./listdanhmuc.component.scss'],
  imports: [
    MatFormFieldModule,
    MatInputModule,
    MatTableModule,
...
export class ListDanhmucComponent implements OnInit {
  ...
  FilterColumns: any[] = JSON.parse(localStorage.getItem('DanhmucColFilter') || '[]');
  Columns: any[] = [];
  ...
  private _DanhmucService: DanhmucService = inject(DanhmucService);
  ...
  page = this._DanhmucService.page;
  pageCount = this._DanhmucService.pageCount;
  total = this._DanhmucService.total;
  pageSize = this._DanhmucService.pageSize;
  danhmucId = this._DanhmucService.danhmucId;
  ...
  constructor() {
    effect(() => {
      this.dataSource.data = this.Listdanhmuc();
  ...
  async ngOnInit(): Promise<void> {
    this._DanhmucService.listenDanhmucUpdates();
    await this._DanhmucService.getAllDanhmuc(this.pageSize(),true);
    this.displayedColumns = Object.keys(this.ColumnName);
    this.dataSource = new MatTableDataSource(this.Listdanhmuc());
    this.dataSource.sort = this.sort;
  ...
  private initializeColumns(): void {
    ...
    localStorage.setItem('DanhmucColFilter', JSON.stringify(this.FilterColumns));
  ...
  async getUpdatedCodeIds() {
    await this._DanhmucService.getUpdatedCodeIds();
  }
  ...
  doFilterHederColumn(event: any, column: any): void {
    this.dataSource.filteredData = this.Listdanhmuc().filter((v: any) => 
  ...
  ResetFilter() {
    this.ListFilter = this.Listdanhmuc();
  }
  ...
  ApplyFilterColum(menu: any) {
    this.dataSource.data = this.Listdanhmuc().filter((v: any) => 
  ...
  DeleteListItem(): void {
    this.EditList.forEach((item: any) => {
      this._DanhmucService.DeleteDanhmuc(item);
    });
    this.EditList = [];
  ...
  goToDetail(item: any): void {
    this.drawer.open();
    this._DanhmucService.setDanhmucId(item.id);
    this._router.navigate(['danhmuc', item.id]);
  }
  ...
  async DoImportData(data: any) {
    ...
    const existingDanhmuc = this._DanhmucService.ListDanhmuc();
    const existingMasp = existingDanhmuc.map((v: any) => v.masp);
    ...
    await Promise.all(uniqueData.map(async (v: any) => {
      const existingItem = existingDanhmuc.find((v1: any) => v1.masp === v.masp);
      if (existingItem) {
        const updatedItem = { ...existingItem, ...v };
        await this._DanhmucService.updateDanhmuc(updatedItem);
      } else {
        await this._DanhmucService.CreateDanhmuc(v);
      }
    }));
    await Promise.all(existingDanhmuc
      .filter(sp => !uniqueData.some((item: any) => item.masp === sp.masp))
      .map(sp => this._DanhmucService.updateDanhmuc({ ...sp, isActive: false }))
    );
  ...
  onPageSizeChange(size: number, menuHienthi: any) {
    ...
    this._DanhmucService.page.set(1);
    this._DanhmucService.getAllDanhmuc(size, true);
    menuHienthi.closeMenu();
  }
  onPreviousPage(){
    if (this.page() > 1) {
      this._DanhmucService.page.set(this.page() - 1);
      this._DanhmucService.getAllDanhmuc(this.pageSize(), true);
    }
  }
