...
export class ListDathangComponent implements OnInit {
  ...
  ColumnName: any = {
    ...
    total: 'Tổng Tiền',
    dathangsanpham: 'Sản Phẩm',
    status: 'Trạng Thái',
    createdAt: 'Ngày Tạo'
  };


  FilterColumns: any[] = JSON.parse(localStorage.getItem('DathangColFilter') || '[]');
  Columns: any[] = [];
  ...
  async ngOnInit(): Promise<void> {
    this._DathangService.listenDathangUpdates();
    await this._DathangService.getAllDathang(this.pageSize(),true);
  ...
  async getUpdatedCodeIds() {
    await this._DathangService.getUpdatedCodeIds();
  }
  ...
  DeleteListItem(): void {
    this.EditList.forEach((item: any) => {
      this._DathangService.DeleteDathang(item);
    });
    this.EditList = [];
  ...
  goToDetail(item: any): void {
    ...
    this._router.navigate(['dathang', item.id]);
  }
  ...
  async DoImportData(data: any) {
    ...
    const existingDathang = this._DathangService.ListDathang();
    const existingMasp = existingDathang.map((v: any) => v.masp);
    ...
    await Promise.all(uniqueData.map(async (v: any) => {
      const existingItem = existingDathang.find((v1: any) => v1.masp === v.masp);
      if (existingItem) {
        const updatedItem = { ...existingItem, ...v };
        await this._DathangService.updateDathang(updatedItem);
      } else {
        await this._DathangService.CreateDathang(v);
      }
    }));
    await Promise.all(existingDathang
      .filter(sp => !uniqueData.some((item: any) => item.masp === sp.masp))
      .map(sp => this._DathangService.updateDathang({ ...sp, isActive: false }))
    );
