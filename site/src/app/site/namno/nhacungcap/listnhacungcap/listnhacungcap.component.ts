...
export class ListNhacungcapComponent implements OnInit {
  ...
  FilterColumns: any[] = JSON.parse(localStorage.getItem('NhacungcapColFilter') || '[]');
  Columns: any[] = [];
  ...
  pageCount = this._NhacungcapService.pageCount;
  ...
  async ngOnInit(): Promise<void> {
    ...
    await this._NhacungcapService.getAllNhacungcap(this.pageSize(),true);
  ...
  async getUpdatedCodeIds() {
    await this._NhacungcapService.getUpdatedCodeIds();
  }
  ...
  doFilterHederColumn(event: any, column: any): void {
    this.dataSource.filteredData = this.Listnhacungcap().filter((v: any) => 
  ...
  ApplyFilterColum(menu: any) {
    this.dataSource.data = this.Listnhacungcap().filter((v: any) => 
  ...
  goToDetail(item: any): void {
    ...
    this._router.navigate(['nhacungcap', item.id]);
  }
  ...
  async DoImportData(data: any) {
    ...
    const existingNhacungcap = this._NhacungcapService.ListNhacungcap();
    const existingMasp = existingNhacungcap.map((v: any) => v.masp);
    ...
    await Promise.all(uniqueData.map(async (v: any) => {
      const existingItem = existingNhacungcap.find((v1: any) => v1.masp === v.masp);
      if (existingItem) {
        const updatedItem = { ...existingItem, ...v };
        await this._NhacungcapService.updateNhacungcap(updatedItem);
      } else {
        await this._NhacungcapService.CreateNhacungcap(v);
      }
    }));
    await Promise.all(existingNhacungcap
      .filter(sp => !uniqueData.some((item: any) => item.masp === sp.masp))
      .map(sp => this._NhacungcapService.updateNhacungcap({ ...sp, isActive: false }))
    );
  ...
  onPageSizeChange(size: number, menuHienthi: any) {
    ...
    this._NhacungcapService.getAllNhacungcap(size, true);
    menuHienthi.closeMenu();
  }
  onPreviousPage(){
    if (this.page() > 1) {
      this._NhacungcapService.page.set(this.page() - 1);
      this._NhacungcapService.getAllNhacungcap(this.pageSize(), true);
    }
  }
