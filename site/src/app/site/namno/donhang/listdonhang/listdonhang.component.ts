...
export class ListDonhangComponent implements OnInit {
  ...
  page = this._DonhangService.page;
  pageCount = this._DonhangService.pageCount;
  total = this._DonhangService.total;
  pageSize = this._DonhangService.pageSize;
  ...
  async ngOnInit(): Promise<void> {
    this._DonhangService.listenDonhangUpdates();
    await this._DonhangService.getAllDonhang(this.pageSize(),true);
  ...
  async getUpdatedCodeIds() {
    await this._DonhangService.getUpdatedCodeIds();
  }
  ...
  goToDetail(item: any): void {
    ...
    this._router.navigate(['donhang', item.id]);
  }
  ...
  async DoImportData(data: any) {
    ...
    const existingDonhang = this._DonhangService.ListDonhang();
    const existingMasp = existingDonhang.map((v: any) => v.masp);
    ...
    await Promise.all(uniqueData.map(async (v: any) => {
      const existingItem = existingDonhang.find((v1: any) => v1.masp === v.masp);
      if (existingItem) {
        const updatedItem = { ...existingItem, ...v };
        await this._DonhangService.updateDonhang(updatedItem);
      } else {
        await this._DonhangService.CreateDonhang(v);
      }
    }));
    await Promise.all(existingDonhang
      .filter(sp => !uniqueData.some((item: any) => item.masp === sp.masp))
      .map(sp => this._DonhangService.updateDonhang({ ...sp, isActive: false }))
    );
  ...
  onPageSizeChange(size: number, menuHienthi: any) {
    ...
    this._DonhangService.page.set(1);
    this._DonhangService.getAllDonhang(size, true);
    menuHienthi.closeMenu();
  }
  onPreviousPage(){
    if (this.page() > 1) {
      this._DonhangService.page.set(this.page() - 1);
      this._DonhangService.getAllDonhang(this.pageSize(), true);
    }
  }
