...
export class ListBanggiaComponent implements OnInit {
  ...
  FilterColumns: any[] = JSON.parse(localStorage.getItem('BanggiaColFilter') || '[]');
  Columns: any[] = [];
  ...
  page = this._BanggiaService.page;
  pageCount = this._BanggiaService.pageCount;
  total = this._BanggiaService.total;
  pageSize = this._BanggiaService.pageSize;
  ...
  async ngOnInit(): Promise<void> {
    this._BanggiaService.listenBanggiaUpdates();
    await this._BanggiaService.getAllBanggia(this.pageSize(),true);
  ...
  async getUpdatedCodeIds() {
    await this._BanggiaService.getUpdatedCodeIds();
  }
  ...
  doFilterHederColumn(event: any, column: any): void {
    this.dataSource.filteredData = this.Listbanggia().filter((v: any) => 
  ...
  ApplyFilterColum(menu: any) {
    this.dataSource.data = this.Listbanggia().filter((v: any) => 
  ...
  DeleteListItem(): void {
    this.EditList.forEach((item: any) => {
      this._BanggiaService.DeleteBanggia(item);
    });
    this.EditList = [];
  ...
  goToDetail(item: any): void {
    ...
    this._router.navigate(['banggia', item.id]);
  }
  ...
  async DoImportData(data: any) {
    ...
    const existingBanggia = this._BanggiaService.ListBanggia();
    const existingMasp = existingBanggia.map((v: any) => v.masp);
    ...
    await Promise.all(uniqueData.map(async (v: any) => {
      const existingItem = existingBanggia.find((v1: any) => v1.masp === v.masp);
      if (existingItem) {
        const updatedItem = { ...existingItem, ...v };
        await this._BanggiaService.updateBanggia(updatedItem);
      } else {
        await this._BanggiaService.CreateBanggia(v);
      }
    }));
    await Promise.all(existingBanggia
      .filter(sp => !uniqueData.some((item: any) => item.masp === sp.masp))
      .map(sp => this._BanggiaService.updateBanggia({ ...sp, isActive: false }))
    );
  ...
  onPageSizeChange(size: number, menuHienthi: any) {
    ...
    this._BanggiaService.page.set(1);
    this._BanggiaService.getAllBanggia(size, true);
    menuHienthi.closeMenu();
  }
  onPreviousPage(){
    if (this.page() > 1) {
      this._BanggiaService.page.set(this.page() - 1);
      this._BanggiaService.getAllBanggia(this.pageSize(), true);
    }
  }
