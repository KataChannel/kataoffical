...
@Component({
  ...
  templateUrl: './listkho.html',
  styleUrls: ['./listkho.scss'],
  imports: [
    MatFormFieldModule,
    MatInputModule,
    MatTableModule,
...
export class ListKhoComponent implements OnInit {
  ...
  FilterColumns: any[] = JSON.parse(localStorage.getItem('KhoColFilter') || '[]');
  Columns: any[] = [];
  ...
  page = this._KhoService.page;
  pageCount = this._KhoService.pageCount;
  total = this._KhoService.total;
  pageSize = this._KhoService.pageSize;
  ...
  constructor() {
    effect(() => {
      this.dataSource.data = this.Listkho();
  ...
  async ngOnInit(): Promise<void> {
    this._KhoService.listenKhoUpdates();
    await this._KhoService.getAllKho(this.pageSize(),true);
    this.displayedColumns = Object.keys(this.ColumnName);
    this.dataSource = new MatTableDataSource(this.Listkho());
    this.dataSource.sort = this.sort;
  ...
  async getUpdatedCodeIds() {
    await this._KhoService.getUpdatedCodeIds();
  }
  ...
  DeleteListItem(): void {
    this.EditList.forEach((item: any) => {
      this._KhoService.DeleteKho(item);
    });
    this.EditList = [];
  ...
  goToDetail(item: any): void {
    ...
    this._router.navigate(['kho', item.id]);
  }
  ...
  onPageSizeChange(size: number, menuHienthi: any) {
    ...
    this._KhoService.page.set(1);
    this._KhoService.getAllKho(size, true);
    menuHienthi.closeMenu();
  }
  onPreviousPage(){
    if (this.page() > 1) {
      this._KhoService.page.set(this.page() - 1);
      this._KhoService.getAllKho(this.pageSize(), true);
    }
  }
