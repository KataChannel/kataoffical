...
import { groupAndCreateChild } from '../../../../shared/utils/group.utils';
import { ListSanpham } from '../../namno';
...
export class ListImportdataComponent implements OnInit {
  ...
  ListImportType = signal<any[]>([
    {id:1,title:'Danh Mục',value:'danhmuc',status:true},
    {id:2,title:'Sản Phẩm',value:'sanpham',status:true},
    {id:3,title:'Khách Hàng',value:'khachhang',status:true},
    {id:4,title:'Nhà Cung Cấp',value:'nhacungcap',status:true},
    {id:5,title:'Bảng Giá',value:'banggia',status:true},
    {id:6,title:'Đơn Hàng',value:'donhang',status:true},
    {id:7,title:'Đặt Hàng',value:'dathang',status:true},
    {id:8,title:'Bảng Giá Khách Hàng',value:'banggiakhachhang',status:true},
    {id:9,title:'Bảng Giá Sản Phẩm',value:'banggiasanpham',status:true},
    {id:10,title:'Nhà Cung Cấp Sản Phẩm',value:'nhacungcapsanpham',status:true},
    {id:11,title:'Xuất Nhập Tồn',value:'xuatnhapton',status:true},
  ]);
  ListEdit = signal<any[]>([]);
  ...
  constructor() {
    ...
    this.rawListDM = this._DanhmucService.ListDanhmuc();
  ...
  async ngOnInit(): Promise<void> {
    const result = ListSanpham.flatMap(item =>
        item.gia.map((v:any) => ({
          codeId: item.codeId,
          title: item.name,
          gia: v.gia,
...
async ExportExcel(title:any) {
    const ListDM = Array.isArray(this.rawListDM) && this.rawListDM.length
      ? this.rawListDM.map((item: any) => ({
          codeId: item.codeId,
          title: item.title,
    ...
    const ListSP = Array.isArray(this.rawListSP) && this.rawListSP.length
      ? this.rawListSP.flatMap(item =>
        item.bacgia.map((v:any) => ({
          codeId: item.codeId,
          title: item.title,
          gia: v.gia,
          max: v.max,
    ...
    const ListTonkho = this.rawListTonkho.map((v: any) => ({
      ...
      if (this.ListEdit().some((item: any) => item.value === 'danhmuc')) {
        Exportdata['danhmuc'] = { data: ListDM };
      }
...
convertNCCSPToImport(data: any){ 
  ...
  async DoImportData(data:any)
  {
    ...
    {
      console.log('san Pham:', data.sanpham);
      ...
      const ListSP = (data.sanpham || []).map((v: any) => ({
        ...
        min: Number(v.min)||0,
        type: v.type
      })).filter((v: any) => v.codeId !== undefined && v.codeId !== null && v.codeId !== '');
      console.log('ListSP:', ListSP);
      const result = groupAndCreateChild(ListSP, 'codeId', 'bacgia',['codeId','title']);
      console.log('Grouped ListSP by codeId:', result);
      ...
      await this._SanphamService.ImportSanpham(result);
