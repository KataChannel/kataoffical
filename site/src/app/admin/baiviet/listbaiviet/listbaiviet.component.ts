...
import { Debounce, memoize } from '../../../shared/utils/decorators';
  @Component({
    ...
    styleUrls: ['./listbaiviet.component.scss'],
    imports: [
      MatFormFieldModule,
      MatInputModule,
      MatTableModule,
  ...
  export class ListBaivietComponent {
    ...
    FilterColumns: any[] = JSON.parse(
      localStorage.getItem('BaivietColFilter') || '[]'
    );
    Columns: any[] = [];
    totalItems = 0;
    pageSize = 10;
    ...
    private _BaivietService: BaivietService = inject(BaivietService);
    private _breakpointObserver: BreakpointObserver = inject(BreakpointObserver);
    private _GoogleSheetService: GoogleSheetService = inject(GoogleSheetService);
    private _router: Router = inject(Router);
    private _dialog: MatDialog = inject(MatDialog);
    Listbaiviet:any = this._BaivietService.ListBaiviet;
    EditList:any=[];
    ...
    constructor() {
      effect(() => {
        this.dataSource.data = this.Listbaiviet();
        this.dataSource.sort = this.sort;
        this.totalItems = this.Listbaiviet().length;
    ...
    async ngOnInit(): Promise<void> {    
      this._BaivietService.listenBaivietUpdates();
      await this._BaivietService.getAllBaiviet();
      this.displayedColumns = Object.keys(this.ColumnName)
      this.updateDisplayData();
      this.dataSource = new MatTableDataSource(this.Listbaiviet());
      this.dataSource.sort = this.sort;
    ...
    private initializeColumns(): void {
      ...
      if (this.FilterColumns.length === 0) {
        ...
        localStorage.setItem('BaivietColFilter',JSON.stringify(this.FilterColumns)
        );
      }
    ...
    doFilterHederColumn(event: any, column: any): void {
      this.dataSource.filteredData = this.Listbaiviet().filter((v: any) => v[column].toLowerCase().includes(event.target.value.toLowerCase()));  
    ...
    ResetFilter()
    {
      this.ListFilter = this.Listbaiviet();
      // this.dataSource.data = this.Listbaiviet();
    ...
    {    
      this.dataSource.data = this.Listbaiviet().filter((v: any) => this.ListFilter.some((v1) => v1.id === v.id));
      this.dataSource.paginator = this.paginator;
      this.dataSource.sort = this.sort;
      menu.closeMenu();
    }
    ...
    DeleteListItem(): void {
      this.EditList.forEach((item: any) => {
        this._BaivietService.DeleteBaiviet(item);
      });
      this.EditList = [];
    ...
    goToDetail(item: any): void {
      this.drawer.open();
      this._BaivietService.setBaivietId(item.id);
    ...
    async DoImportData(data: any) {
      ...
      const existingBaiviet = this._BaivietService.ListBaiviet();
      const existingMasp  = existingBaiviet.map((v: any) => v.masp);
      ...
      await Promise.all(uniqueData.map(async (v: any) => {
        const existingItem = existingBaiviet.find((v1: any) => v1.masp === v.masp);
        if (existingItem) {
          const updatedItem = { ...existingItem, ...v };
          await this._BaivietService.updateBaiviet(updatedItem);
        } else {
          await this._BaivietService.CreateBaiviet(v);
        }
      }));
      await Promise.all(existingBaiviet
        .filter(sp => !uniqueData.some((item:any) => item.masp === sp.masp))
        .map(sp => this._BaivietService.updateBaiviet({ ...sp, isActive: false }))
      );
  ...
  onPageSizeChange(size: number,menuHienthi:any) {
    if(size>this.Listbaiviet().length){
      this.pageSize = this.Listbaiviet().length;
      this._snackBar.open(`Số lượng tối đa ${this.Listbaiviet().length}`, '', {
        duration: 1000,
        horizontalPosition: "end",
        verticalPosition: "top",
        panelClass: ['snackbar-success'],
  ...
  updateDisplayData() {
    ...
    const pageData = this.Listbaiviet().slice(startIndex, endIndex);
