...
import { StorageService } from '../../../shared/utils/storage.service';
  ...
  export class ListHoadonComponent {
    ...
    FilterColumns: any[] = JSON.parse(
      localStorage.getItem('HoadonColFilter') || '[]'
    );
    Columns: any[] = [];
    totalItems = 0;
    pageSize = 10;
    ...
    Listhoadon:any = this._HoadonService.ListHoadon;
    EditList:any=[];
    ...
    constructor() {
      effect(() => {
        ...
        this.totalItems = this.Listhoadon().length;
    ...
    async fetchData() {
      ...
      for (let month = 0; month < 12; month++) {
        const startDay = 1;
        const daysInMonth = new Date(Number(this.Detail.nam), month + 1, 0).getDate();
        const batdau = `${String(startDay).padStart(2, '0')}/${String(month + 1).padStart(2, '0')}/${this.Detail.nam}T00:00:00`;
        const ketthuc = `${String(daysInMonth).padStart(2, '0')}/${String(month + 1).padStart(2, '0')}/${this.Detail.nam}T23:59:59`;
    ...
    doFilterHederColumn(event: any, column: any): void {
      this.dataSource.filteredData = this.Listhoadon().filter((v: any) => v[column].toLowerCase().includes(event.target.value.toLowerCase()));  
    ...
    {    
      this.dataSource.data = this.Listhoadon().filter((v: any) => this.ListFilter.some((v1) => v1.id === v.id));
      this.dataSource.paginator = this.paginator;
      this.dataSource.sort = this.sort;
      menu.closeMenu();
    }
  ...
  onPageSizeChange(size: number,menuHienthi:any) {
    if(size>this.Listhoadon().length){
      this.pageSize = this.Listhoadon().length;
      this._snackBar.open(`Số lượng tối đa ${this.Listhoadon().length}`, '', {
        duration: 1000,
        horizontalPosition: "end",
        verticalPosition: "top",
        panelClass: ['snackbar-success'],
  ...
  updateDisplayData() {
    ...
    const pageData = this.Listhoadon().slice(startIndex, endIndex);
