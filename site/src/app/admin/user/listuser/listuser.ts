...
export class ListUserComponent implements OnInit {
  ...
  FilterColumns: any[] = JSON.parse(localStorage.getItem('UserColFilter') || '[]');
  Columns: any[] = [];
  ...
  page = this._UserService.page;
  totalPages = this._UserService.totalPages;
  total = this._UserService.total;
  pageSize = this._UserService.pageSize;
  ...
  constructor() {
    effect(async () => {
      console.log('effect',this.Listuser());
  ...
  async ngOnInit(): Promise<void> {
    ...
    await this._UserService.getAllUser(this.searchParam);
    console.log('ngOnInit',this.Listuser());
  ...
  async getUpdatedCodeIds() {
    await this._UserService.getUpdatedCodeIds();
  }
  ...
  async onPageSizeChange(size: number, menuHienthi: any) {
    ...
    this._UserService.page.set(1);
    this._UserService.pageSize.set(size);
    await this._UserService.getAllUser(this.searchParam, true);
    menuHienthi.closeMenu();
  }
  async onPreviousPage(){
    if (this.page() > 1) {
      this._UserService.page.set(this.page() - 1);
      await this._UserService.getAllUser(this.searchParam, true);
      console.log('Previous Page', this.Listuser());
    }
  }
  ...
  async onNextPage(){
    if (this.page() < this.totalPages()) {
      this._UserService.page.set(this.page() + 1);
      await this._UserService.getAllUser(this.searchParam, true);
      console.log('Next Page', this.Listuser());
    }
  }
}
