...
@Component({
  ...
  styleUrl: './mathang.component.scss',
  imports: [
    MatFormFieldModule,
    MatInputModule,
    MatTableModule,
...
async setisProduct(): Promise<void> {
  ...
  try {
    for (const item of this.EditList) {
      ...
      await this._HoadonchitietService.updateMathang(item);
    }
    this._snackBar.open('Cập Nhật Thành Công', '', {
      duration: 1000,
      horizontalPosition: 'end',
  ...
  async onPageChange(event: any): Promise<void> {    
    this.SearchParams.page = Number(event.page);
    this.SearchParams.pageSize = Number(event.pageSize);
    await this._HoadonchitietService.getMathang(this.SearchParams);
  }
