...
export class ListHoadonchitietComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    await this._HoadonchitietService.getAllHoadonchitiet(this.pageSize(),true);
...
async fetchData() {
    ...
    for (const item of listHoadon) {
      const result = await this._HoadonchitietService.fetchData({
        token: this.hoadon_token,
        ...item,
      });
      // Increase delay between each hoadon call to avoid error 429
        ...
        if (result.hdhhdvu && Array.isArray(result.hdhhdvu)) {
          ...
          worker.onmessage = (event: MessageEvent<any>) => {
            if (event.data.status === 'success') {
              console.log('Created via worker:', event.data.item);
            } else if (event.data.status === 'error') {
              console.error('Error via worker:', event.data.error, event.data.item);
            } else if (event.data.status === 'done') {
              console.log('Worker completed processing items.');
              worker.terminate();
            }
          };
