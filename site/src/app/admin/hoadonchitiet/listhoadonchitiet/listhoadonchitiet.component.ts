...
import { HoadonService } from '../../hoadon/hoadon.service';
  @Component({
    ...
    templateUrl: './listhoadonchitiet.component.html',
    styleUrls: ['./listhoadonchitiet.component.scss'],
    imports: [
      MatFormFieldModule,
      MatInputModule,
      MatTableModule,
  ...
  export class ListHoadonchitietComponent {
    ...
    FilterColumns: any[] = JSON.parse(
      localStorage.getItem('HoadonchitietColFilter') || '[]'
    );
    Columns: any[] = [];
    totalItems = 0;
    pageSize = 10;
    ...
    Listhoadonchitiet:any = this._HoadonchitietService.ListHoadonchitiet;
    EditList:any=[];
    ...
    token: any= this._StorageService.getItem('hoadon_token') || '';
    constructor() {
      effect(() => {
        this.dataSource.data = this.Listhoadonchitiet();        
        this.totalItems = this.Listhoadonchitiet().length;
    ...
    async ngOnInit(): Promise<void> {    
      ...
      await this._HoadonchitietService.getAllHoadonchitiet();
    ...
    private initializeColumns(): void {
      ...
      if (this.FilterColumns.length === 0) {
        ...
        localStorage.setItem('HoadonchitietColFilter',JSON.stringify(this.FilterColumns)
        );
      }
    ...
    doFilterHederColumn(event: any, column: any): void {
      this.dataSource.filteredData = this.Listhoadonchitiet().filter((v: any) => v[column].toLowerCase().includes(event.target.value.toLowerCase()));  
    ...
    {    
      this.dataSource.data = this.Listhoadonchitiet().filter((v: any) => this.ListFilter.some((v1) => v1.id === v.id));
      this.dataSource.paginator = this.paginator;
      this.dataSource.sort = this.sort;
      menu.closeMenu();
    }
  ...
  onPageSizeChange(size: number,menuHienthi:any) {
    if(size>this.Listhoadonchitiet().length){
      this.pageSize = this.Listhoadonchitiet().length;
      this._snackBar.open(`Số lượng tối đa ${this.Listhoadonchitiet().length}`, '', {
        duration: 1000,
        horizontalPosition: "end",
        verticalPosition: "top",
        panelClass: ['snackbar-success'],
  ...
  updateDisplayData() {
    ...
    const pageData = this.Listhoadonchitiet().slice(startIndex, endIndex);
