...
@Component({
  ...
  styleUrls: ['./listsetting.component.scss'],
  imports: [
    MatFormFieldModule,
    MatInputModule,
    MatTableModule,
...
export class ListSettingComponent implements OnInit {
  ...
  FilterColumns: any[] = JSON.parse(localStorage.getItem('SettingColFilter') || '[]');
  Columns: any[] = [];
  ...
  private _SettingService: SettingService = inject(SettingService);
  ...
  Listsetting = this._SettingService.ListSetting;
  page = this._SettingService.page;
  pageCount = this._SettingService.pageCount;
  total = this._SettingService.total;
  pageSize = this._SettingService.pageSize;
  settingId = this._SettingService.settingId;
  ...
  constructor() {
    effect(async () => {
      await this._SettingService.getSettingBy(this.searchParam);
  ...
  async getUpdatedCodeIds() {
    await this._SettingService.getUpdatedCodeIds();
  }
  ...
  doFilterHederColumn(event: any, column: any): void {
    this.dataSource.filteredData = this.Listsetting().filter((v: any) => 
  ...
  ApplyFilterColum(menu: any) {
    this.dataSource.data = this.Listsetting().filter((v: any) => 
  ...
  private updateDisplayedColumns(): void {
    this.displayedColumns = this.FilterColumns.filter((v) => v.isShow && v.key !== 'stt').map((item) => item.key);
  ...
  DeleteListItem(): void {
    this.EditList.forEach((item: any) => {
      this._SettingService.DeleteSetting(item);
    });
    this.EditList = [];
  ...
  goToDetail(item: any): void {
    this.drawer.open();
    this._SettingService.setSettingId(item.id);
    this._router.navigate(['admin/setting', item.id]);
  }
  ...
  async DoImportData(data: any) {
    ...
    const existingSetting = this._SettingService.ListSetting();
    const existingMasp = existingSetting.map((v: any) => v.masp);
    ...
    await Promise.all(uniqueData.map(async (v: any) => {
      const existingItem = existingSetting.find((v1: any) => v1.masp === v.masp);
      if (existingItem) {
        const updatedItem = { ...existingItem, ...v };
        await this._SettingService.updateSetting(updatedItem);
      } else {
        await this._SettingService.CreateSetting(v);
      }
    }));
    await Promise.all(existingSetting
      .filter(sp => !uniqueData.some((item: any) => item.masp === sp.masp))
      .map(sp => this._SettingService.updateSetting({ ...sp, isActive: false }))
    );
  ...
  onPageSizeChange(size: number, menuHienthi: any) {
    ...
    this._SettingService.page.set(1);
    this._SettingService.getAllSetting(size, true);
    menuHienthi.closeMenu();
  }
  onPreviousPage(){
    if (this.page() > 1) {
      this._SettingService.page.set(this.page() - 1);
      this._SettingService.getAllSetting(this.pageSize(), true);
    }
  }
