...
export class KataTableComponent {
  ...
  toggleColumn(key: string) {
    // Prevent toggling the select checkbox column
    if (key === 'select') {
      return;
    }
    // Toggle visibility of the column
    this.visibleColumns[key] = !this.visibleColumns[key];
    // Ensure at least one non-select column remains visible
    const visibleNonSelect = Object.keys(this.visibleColumns)
      .filter(k => k !== 'select')
      .filter(k => this.visibleColumns[k]).length;
    if (visibleNonSelect === 0) {
      // Revert toggle if no non-select columns would remain
  ...
  // Columns
  get columns(): Column[] {
    // Lấy width động từ columnWidths (nếu width thay đổi)
    return [
