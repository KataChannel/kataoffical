...
export class HoahongService {
  ...
  async getLastUpdatedHoahong(): Promise<{ updatedAt: number }> {
    try {
  ...
  async getTotalHoahongByUserId(userId: string) {
    try {
  ...
  async findOne(id: string) {
    try {
      ...
      if (!item) throw new NotFoundException('Hoahong not found');
      return item;
    } catch (error) {
  ...
  async reorderHoahongs(hoahongIds: string[]) {
    try {
