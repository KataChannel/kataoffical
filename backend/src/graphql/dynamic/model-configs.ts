import { ModelConfig, ModelRegistry } from './dynamic-resolver.factory';

// Define all model configurations based on Prisma schema
export const MODEL_CONFIGURATIONS: ModelConfig[] = [
  {
    name: 'User',
    pluralName: 'users',
    fields: {
...
export function registerAllModels() {
  MODEL_CONFIGURATIONS.forEach(config => {
...
export function getModelConfig(modelName: string): ModelConfig | undefined {
  return MODEL_CONFIGURATIONS.find(config => 
    config.name === modelName || config.name.toLowerCase() === modelName.toLowerCase()
  );
}