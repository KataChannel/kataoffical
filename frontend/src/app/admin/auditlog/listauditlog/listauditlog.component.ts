...
@Component({
  ...
  styleUrls: ['./listauditlog.component.scss'],
  imports: [
    MatFormFieldModule,
    MatInputModule,
    MatTableModule,
...
export class ListAuditlogComponent implements OnInit {
  ...
  private _AuditlogService: AuditlogService = inject(AuditlogService);
  ...
  async getUpdatedCodeIds() {
    await this._AuditlogService.getUpdatedCodeIds();
  }
  ...
  doFilterHederColumn(event: any, column: any): void {
    this.dataSource.filteredData = this.Listauditlog().filter((v: any) => 
  ...
  async DoImportData(data: any) {
    ...
    const existingMasp = existingAuditlog.map((v: any) => v.masp);
    ...
    await Promise.all(uniqueData.map(async (v: any) => {
      const existingItem = existingAuditlog.find((v1: any) => v1.masp === v.masp);
      if (existingItem) {
        const updatedItem = { ...existingItem, ...v };
        await this._AuditlogService.updateAuditlog(updatedItem);
      } else {
        await this._AuditlogService.CreateAuditlog(v);
      }
    }));
    await Promise.all(existingAuditlog
      .filter(sp => !uniqueData.some((item: any) => item.masp === sp.masp))
      .map(sp => this._AuditlogService.updateAuditlog({ ...sp, isActive: false }))
    );
