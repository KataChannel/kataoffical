...
@Component({
  selector: 'app-listkhoahoc',
  templateUrl: './listkhoahoc.html',
  styleUrls: ['./listkhoahoc.scss'],
  imports: [
    MatFormFieldModule,
    MatInputModule,
    MatTableModule,
...
export class ListKhoahocComponent implements OnInit {
  ...
  FilterColumns: any[] = JSON.parse(localStorage.getItem('KhoahocColFilter') || '[]');
  Columns: any[] = [];
  ...
  private _KhoahocService: KhoahocService = inject(KhoahocService);
  private _breakpointObserver: BreakpointObserver = inject(BreakpointObserver);
  private _router: Router = inject(Router);
  private _dialog: MatDialog = inject(MatDialog);
  private _snackBar: MatSnackBar = inject(MatSnackBar);

  Listkhoahoc = this._KhoahocService.ListKhoahoc;
  page = this._KhoahocService.page;
  totalPages = this._KhoahocService.totalPages;
  total = this._KhoahocService.total;
  pageSize = this._KhoahocService.pageSize;
  khoahocId = this._KhoahocService.khoahocId;
  ...
  constructor() {
    effect(() => {
      this.dataSource.data = this.Listkhoahoc();
  ...
  async ngOnInit(): Promise<void> {
    this._KhoahocService.listenKhoahocUpdates();
    await this._KhoahocService.getAllKhoahoc(this.searchParam);
    this.displayedColumns = Object.keys(this.ColumnName);
    this.dataSource = new MatTableDataSource(this.Listkhoahoc());
    this.dataSource.sort = this.sort;
  ...
  private initializeColumns(): void {
    ...
    localStorage.setItem('KhoahocColFilter', JSON.stringify(this.FilterColumns));
  ...
  async getUpdatedCodeIds() {
    await this._KhoahocService.getUpdatedCodeIds();
  }
  ...
  DeleteListItem(): void {
    this.EditList.forEach((item: any) => {
      this._KhoahocService.DeleteKhoahoc(item);
    });
    this.EditList = [];
  ...
  goToDetail(item: any): void {
    this.drawer.open();
    this._KhoahocService.setKhoahocId(item.id);
    this._router.navigate(['admin/khoahoc', item.id]);
  }
  ...
  onPageSizeChange(size: number, menuHienthi: any) {
    ...
    this._KhoahocService.page.set(1);
    this._KhoahocService.getAllKhoahoc(this.searchParam, true);
    menuHienthi.closeMenu();
  }
  onPreviousPage(){
    if (this.page() > 1) {
      this._KhoahocService.page.set(this.page() - 1);
      this._KhoahocService.getAllKhoahoc(this.searchParam, true);
    }
  }
