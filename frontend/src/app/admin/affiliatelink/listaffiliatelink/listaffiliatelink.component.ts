import { AfterViewInit, ChangeDetectionStrategy, Component, computed, effect, inject, TemplateRef, ViewChild } from '@angular/core';
  ...
  export class ListAffiliatelinkComponent {
    ...
    FilterColumns: any[] = JSON.parse(
      localStorage.getItem('AffiliatelinkColFilter') || '[]'
    );
    Columns: any[] = [];
    totalItems = 0;
    pageSize = 10;
    ...
    Listaffiliatelink:any = this._AffiliatelinkService.ListAffiliatelink;
    EditList:any=[];
    ...
    constructor() {
      effect(() => {
        ...
        console.log(this.Listaffiliatelink());
        
        this.totalItems = this.Listaffiliatelink().length;
    ...
    async ngOnInit(): Promise<void> {    
      ...
      await this._AffiliatelinkService.getAllAffiliatelink();
    ...
    private initializeColumns(): void {
      ...
      if (this.FilterColumns.length === 0) {
        ...
        localStorage.setItem('AffiliatelinkColFilter',JSON.stringify(this.FilterColumns)
        );
      }
    ...
    doFilterHederColumn(event: any, column: any): void {
      this.dataSource.filteredData = this.Listaffiliatelink().filter((v: any) => v[column].toLowerCase().includes(event.target.value.toLowerCase()));  
    ...
    {    
      this.dataSource.data = this.Listaffiliatelink().filter((v: any) => this.ListFilter.some((v1) => v1.id === v.id));
      this.dataSource.paginator = this.paginator;
      this.dataSource.sort = this.sort;
      menu.closeMenu();
    }
  ...
  onPageSizeChange(size: number,menuHienthi:any) {
    if(size>this.Listaffiliatelink().length){
      this.pageSize = this.Listaffiliatelink().length;
      this._snackBar.open(`Số lượng tối đa ${this.Listaffiliatelink().length}`, '', {
        duration: 1000,
        horizontalPosition: "end",
        verticalPosition: "top",
        panelClass: ['snackbar-success'],
  ...
  updateDisplayData() {
    ...
    const pageData = this.Listaffiliatelink().slice(startIndex, endIndex);
