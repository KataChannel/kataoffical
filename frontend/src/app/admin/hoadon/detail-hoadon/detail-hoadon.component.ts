...
@Component({
  ...
  template: `
    <div class="container mx-auto px-4 py-6 sm:px-6 lg:px-8 max-w-4xl">
      ...
      <div class="flex flex-col gap-4 mb-6 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-muted-foreground" *ngIf="hoadon()">
              Số: {{ hoadon()?.soHoaDon }} | Đơn: {{ hoadon()?.donhang?.madonhang }}
            </p>
          </div>
        </div>
        
        <div class="flex gap-2" *ngIf="hoadon()">
           <ui-button *ngIf="hoadon()?.pdfUrl" variant="default" (click)="viewPDF()">
              📄 Xem PDF
           </ui-button>
           <ui-button *ngIf="hoadon()?.trangThai === 'NHAP'" variant="success" (click)="approve()">
              ✅ Duyệt & Xuất
           </ui-button>
        </div>
      </div>
      ...
      <!-- Main Content -->
      <div *ngIf="!loading() && hoadon()" class="space-y-6">
        <!-- Info Card -->
        <ui-card>
          <ui-card-content class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div class="space-y-4 bg-slate-50 p-4 rounded-lg">
                ...
                <div class="flex justify-between items-center" *ngIf="hoadon()?.ngayDuyet">
          ...
          <ui-card-content class="p-0 overflow-hidden">
            <table class="w-full text-sm">
              ...
              <tbody class="divide-y divide-slate-100">
                <tr *ngFor="let item of hoadon()?.details; let i = index">
