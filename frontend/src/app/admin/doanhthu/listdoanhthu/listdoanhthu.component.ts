...
@Component({
  selector: 'app-listdoanhthu',
  templateUrl: './listdoanhthu.component.html',
  styleUrls: ['./listdoanhthu.component.scss'],
  imports: [
    MatFormFieldModule,
    MatInputModule,
    MatTableModule,
...
export class ListDoanhthuComponent implements OnInit {
  ...
  FilterColumns: any[] = JSON.parse(localStorage.getItem('DoanhthuColFilter') || '[]');
  Columns: any[] = [];
  ...
  private _DoanhthuService: DoanhthuService = inject(DoanhthuService);
  ...
  Listdoanhthu = this._DoanhthuService.ListDoanhthu;
  page = this._DoanhthuService.page;
  pageCount = this._DoanhthuService.pageCount;
  total = this._DoanhthuService.total;
  pageSize = this._DoanhthuService.pageSize;
  doanhthuId = this._DoanhthuService.doanhthuId;
  ...
  constructor() {
    effect(() => {
      this.dataSource.data = this.Listdoanhthu();
  ...
  async ngOnInit(): Promise<void> {
    this._DoanhthuService.listenDoanhthuUpdates();
    await this._DoanhthuService.getAllDoanhthu(this.pageSize(),true);
    this.displayedColumns = Object.keys(this.ColumnName);
    this.dataSource = new MatTableDataSource(this.Listdoanhthu());
    this.dataSource.sort = this.sort;
  ...
  private initializeColumns(): void {
    ...
    localStorage.setItem('DoanhthuColFilter', JSON.stringify(this.FilterColumns));
  ...
  async getUpdatedCodeIds() {
    await this._DoanhthuService.getUpdatedCodeIds();
  }
  ...
  doFilterHederColumn(event: any, column: any): void {
    this.dataSource.filteredData = this.Listdoanhthu().filter((v: any) => 
  ...
  ResetFilter() {
    this.ListFilter = this.Listdoanhthu();
  }
  ...
  ApplyFilterColum(menu: any) {
    this.dataSource.data = this.Listdoanhthu().filter((v: any) => 
  ...
  DeleteListItem(): void {
    this.EditList.forEach((item: any) => {
      this._DoanhthuService.DeleteDoanhthu(item);
    });
    this.EditList = [];
  ...
  goToDetail(item: any): void {
    this.drawer.open();
    this._DoanhthuService.setDoanhthuId(item.id);
    this._router.navigate(['admin/doanhthu', item.id]);
  }
  ...
  async DoImportData(data: any) {
    ...
    const existingDoanhthu = this._DoanhthuService.ListDoanhthu();
    const existingMasp = existingDoanhthu.map((v: any) => v.masp);
    ...
    await Promise.all(uniqueData.map(async (v: any) => {
      const existingItem = existingDoanhthu.find((v1: any) => v1.masp === v.masp);
      if (existingItem) {
        const updatedItem = { ...existingItem, ...v };
        await this._DoanhthuService.updateDoanhthu(updatedItem);
      } else {
        await this._DoanhthuService.CreateDoanhthu(v);
      }
    }));
    await Promise.all(existingDoanhthu
      .filter(sp => !uniqueData.some((item: any) => item.masp === sp.masp))
      .map(sp => this._DoanhthuService.updateDoanhthu({ ...sp, isActive: false }))
    );
  ...
  onPageSizeChange(size: number, menuHienthi: any) {
    ...
    this._DoanhthuService.page.set(1);
    this._DoanhthuService.getAllDoanhthu(size, true);
    menuHienthi.closeMenu();
  }
  onPreviousPage(){
    if (this.page() > 1) {
      this._DoanhthuService.page.set(this.page() - 1);
      this._DoanhthuService.getAllDoanhthu(this.pageSize(), true);
    }
  }
