...
@Component({
  ...
  styleUrls: ['./listquanlyqrcode.component.scss'],
  imports: [
    MatFormFieldModule,
    MatInputModule,
    MatTableModule,
...
export class ListQuanlyqrcodeComponent {
  ...
  private initializeColumns(): void {
    ...
    if (this.FilterColumns.length === 0) {
      ...
      localStorage.setItem('QuanlyqrcodeColFilter',JSON.stringify(this.FilterColumns)
      );
    }
  ...
  doFilterHederColumn(event: any, column: any): void {
    this.dataSource.filteredData = this.Listquanlyqrcode().filter((v: any) => v[column].toLowerCase().includes(event.target.value.toLowerCase()));  
  ...
  ResetFilter()
  {
    this.ListFilter = this.Listquanlyqrcode();
    this.dataSource.data = this.Listquanlyqrcode();
    this.dataSource.paginator = this.paginator;
    this.dataSource.sort = this.sort;
  }
  ...
  {    
    this.dataSource.data = this.Listquanlyqrcode().filter((v: any) => this.ListFilter.some((v1) => v1.id === v.id));
    this.dataSource.paginator = this.paginator;
    this.dataSource.sort = this.sort;
    menu.closeMenu();
  }
  ...
  DeleteListItem(): void {
    this.EditList.forEach((item: any) => {
      this._QuanlyqrcodeService.DeleteQuanlyqrcode(item);
    });
    this.EditList = [];
  ...
  goToDetail(item: any): void {
    this.drawer.open();
    this._QuanlyqrcodeService.setQuanlyqrcodeId(item.id);
    this._router.navigate(['admin/quanlyqrcode', item.id]);
  }
  async LoadDrive() {
    const DriveInfo = {
  ...
  async DoImportData(data: any) {
    ...
    const existingQuanlyqrcode = this._QuanlyqrcodeService.ListQuanlyqrcode();
    const existingMasp  = existingQuanlyqrcode.map((v: any) => v.masp);
    ...
    await Promise.all(uniqueData.map(async (v: any) => {
      const existingItem = existingQuanlyqrcode.find((v1: any) => v1.masp === v.masp);
      if (existingItem) {
        const updatedItem = { ...existingItem, ...v };
        await this._QuanlyqrcodeService.updateQuanlyqrcode(updatedItem);
      } else {
        await this._QuanlyqrcodeService.CreateQuanlyqrcode(v);
      }
    }));
    await Promise.all(existingQuanlyqrcode
      .filter(sp => !uniqueData.some((item:any) => item.masp === sp.masp))
      .map(sp => this._QuanlyqrcodeService.updateQuanlyqrcode({ ...sp, isActive: false }))
    );
...
onPageSizeChange(size: number,menuHienthi:any) {
  if(size>this.Listquanlyqrcode().length){
    this.pageSize = this.Listquanlyqrcode().length;
    this._snackBar.open(`Số lượng tối đa ${this.Listquanlyqrcode().length}`, '', {
      duration: 1000,
      horizontalPosition: "end",
      verticalPosition: "top",
      panelClass: ['snackbar-success'],
...
updateDisplayData() {
  ...
  const pageData = this.Listquanlyqrcode().slice(startIndex, endIndex);
