      ...
      <div class="px-6 py-4 border-b border-gray-50 flex items-center justify-between bg-gradient-to-r from-gray-50/50 to-transparent">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            @if (!loading()) {
              <p class="text-xs text-gray-500 font-medium">Bạn có {{ total() }} đơn hàng mới</p>
            }
          </div>
        </div>
      ...
      <div class="p-4">
        ...
        @if (loading()) {
          <div class="space-y-4">
            @for (_ of [1,2,3]; track _) {
              <div class="p-4 border border-gray-50 rounded-xl space-y-3">
        ...
        @if (!loading() && !error() && total() > 0) {
          <div class="space-y-3 max-h-[400px] overflow-y-auto pr-1">
            @for (donhang of list(); track donhang; let i = $index) {
              <div
