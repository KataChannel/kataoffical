...
@Component({
  selector: 'app-listdoanhso',
  templateUrl: './listdoanhso.component.html',
  styleUrls: ['./listdoanhso.component.scss'],
  imports: [
    MatFormFieldModule,
    MatInputModule,
    MatTableModule,
...
export class ListDoanhsoComponent implements OnInit {
  ...
  FilterColumns: any[] = JSON.parse(localStorage.getItem('DoanhsoColFilter') || '[]');
  Columns: any[] = [];
  ...
  private _DoanhsoService: DoanhsoService = inject(DoanhsoService);
  ...
  Listdoanhso = this._DoanhsoService.ListDoanhso;
  page = this._DoanhsoService.page;
  pageCount = this._DoanhsoService.pageCount;
  total = this._DoanhsoService.total;
  pageSize = this._DoanhsoService.pageSize;
  doanhsoId = this._DoanhsoService.doanhsoId;
  ...
  constructor() {
    effect(() => {
      this.dataSource.data = this.Listdoanhso();
  ...
  async ngOnInit(): Promise<void> {
    this._DoanhsoService.listenDoanhsoUpdates();
    await this._DoanhsoService.getAllDoanhso(this.pageSize(),true);
    this.displayedColumns = Object.keys(this.ColumnName);
    this.dataSource = new MatTableDataSource(this.Listdoanhso());
    this.dataSource.sort = this.sort;
  ...
  private initializeColumns(): void {
    ...
    localStorage.setItem('DoanhsoColFilter', JSON.stringify(this.FilterColumns));
  ...
  async getUpdatedCodeIds() {
    await this._DoanhsoService.getUpdatedCodeIds();
  }
  ...
  doFilterHederColumn(event: any, column: any): void {
    this.dataSource.filteredData = this.Listdoanhso().filter((v: any) => 
  ...
  ResetFilter() {
    this.ListFilter = this.Listdoanhso();
  }
  ...
  ApplyFilterColum(menu: any) {
    this.dataSource.data = this.Listdoanhso().filter((v: any) => 
  ...
  DeleteListItem(): void {
    this.EditList.forEach((item: any) => {
      this._DoanhsoService.DeleteDoanhso(item);
    });
    this.EditList = [];
  ...
  goToDetail(item: any): void {
    this.drawer.open();
    this._DoanhsoService.setDoanhsoId(item.id);
    this._router.navigate(['admin/doanhso', item.id]);
  }
  ...
  async DoImportData(data: any) {
    ...
    const existingDoanhso = this._DoanhsoService.ListDoanhso();
    const existingMasp = existingDoanhso.map((v: any) => v.masp);
    ...
    await Promise.all(uniqueData.map(async (v: any) => {
      const existingItem = existingDoanhso.find((v1: any) => v1.masp === v.masp);
      if (existingItem) {
        const updatedItem = { ...existingItem, ...v };
        await this._DoanhsoService.updateDoanhso(updatedItem);
      } else {
        await this._DoanhsoService.CreateDoanhso(v);
      }
    }));
    await Promise.all(existingDoanhso
      .filter(sp => !uniqueData.some((item: any) => item.masp === sp.masp))
      .map(sp => this._DoanhsoService.updateDoanhso({ ...sp, isActive: false }))
    );
  ...
  onPageSizeChange(size: number, menuHienthi: any) {
    ...
    this._DoanhsoService.page.set(1);
    this._DoanhsoService.getAllDoanhso(size, true);
    menuHienthi.closeMenu();
  }
  onPreviousPage(){
    if (this.page() > 1) {
      this._DoanhsoService.page.set(this.page() - 1);
      this._DoanhsoService.getAllDoanhso(this.pageSize(), true);
    }
  }
