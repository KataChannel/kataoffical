...
@Component({
  ...
  templateUrl: './listnhomncc.html',
  styleUrls: ['./listnhomncc.scss'],
  imports: [
    MatFormFieldModule,
    MatInputModule,
    MatTableModule,
...
export class ListNhomnccComponent implements OnInit {
  ...
  FilterColumns: any[] = JSON.parse(localStorage.getItem('NhomnccColFilter') || '[]');
  Columns: any[] = [];
  ...
  async ngOnInit(): Promise<void> {
   this.Listnhomncc = await this.getAllNhomncc();
    console.log('Listnhomncc', this.Listnhomncc);
  ...
  DeleteListItem(): void {
    this.EditList.forEach((item: any) => {
      // this._NhomnccService.DeleteNhomncc(item);
    });
    this.EditList = [];
  ...
  onPageSizeChange(size: number, menuHienthi: any) {
    ...
    // this._NhomnccService.page.set(1);
    // this._NhomnccService.getAllNhomncc(this.searchParam, true);
    menuHienthi.closeMenu();
  }
  onPreviousPage(){
    if (this.page() > 1) {
      // this._NhomnccService.page.set(this.page() - 1);
      // this._NhomnccService.getAllNhomncc(this.searchParam, true);
    }
  }
