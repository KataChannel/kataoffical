...
export class DashboardService {
  ...
  /**
   * Get dashboard statistics with proper date handling
   */
  async getDashboardStats(filters?: {
    ...
    try {
      ...
      const response = await this.graphqlService.getDashboardStats(dateRange);
      ...
      if (response.data?.dashboardStats) {
        this.dashboardData.set(response.data.dashboardStats);
  ...
  /**
   * Get default date range for dashboard (last 30 days)
   */
