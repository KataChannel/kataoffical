import { AfterViewInit, ChangeDetectionStrategy, Component, computed, effect, inject, TemplateRef, ViewChild } from '@angular/core';
  ...
  export class ListTonkhoComponent {
    ...
    private _TonkhoService: TonkhoService = inject(TonkhoService);
    private _breakpointObserver: BreakpointObserver = inject(BreakpointObserver);
    private _GoogleSheetService: GoogleSheetService = inject(GoogleSheetService);
    private _router: Router = inject(Router);
    private _dialog: MatDialog = inject(MatDialog);
    Listtonkho:any = this._TonkhoService.ListTonkho;
    EditList:any=[];
    ...
    constructor() {
      effect(() => {
        this.dataSource.data = this.Listtonkho();
        console.log(this.Listtonkho());
        
        this.totalItems = this.Listtonkho().length;
    ...
    async ngOnInit(): Promise<void> {    
      this._TonkhoService.listenTonkhoUpdates();
      await this._TonkhoService.getAllTonkho();
      this.displayedColumns = Object.keys(this.ColumnName)
      console.log(this.displayedColumns);
      this.updateDisplayData();
      this.dataSource = new MatTableDataSource(this.Listtonkho());
    ...
    doFilterHederColumn(event: any, column: any): void {
      this.dataSource.filteredData = this.Listtonkho().filter((v: any) => v[column].toLowerCase().includes(event.target.value.toLowerCase()));  
    ...
    ResetFilter()
    {
      this.ListFilter = this.Listtonkho();
      // this.dataSource.data = this.Listtonkho();
      // this.dataSource.paginator = this.paginator;
      // this.dataSource.sort = this.sort;
    }
    ...
    {    
      this.dataSource.data = this.Listtonkho().filter((v: any) => this.ListFilter.some((v1) => v1.id === v.id));
      this.dataSource.paginator = this.paginator;
      this.dataSource.sort = this.sort;
      menu.closeMenu();
    }
    ...
    DeleteListItem(): void {
      this.EditList.forEach((item: any) => {
        this._TonkhoService.DeleteTonkho(item);
      });
      this.EditList = [];
    ...
    goToDetail(item: any): void {
      this.drawer.open();
      this._TonkhoService.setTonkhoId(item.id);
      this._router.navigate(['admin/tonkho', item.id]);
    }
    ...
    async DoImportData(data: any) {
      ...
      const existingTonkho = this._TonkhoService.ListTonkho();
      const existingMasp  = existingTonkho.map((v: any) => v.masp);
      ...
      await Promise.all(uniqueData.map(async (v: any) => {
        const existingItem = existingTonkho.find((v1: any) => v1.masp === v.masp);
        if (existingItem) {
          const updatedItem = { ...existingItem, ...v };
          await this._TonkhoService.updateTonkho(updatedItem);
        } else {
          await this._TonkhoService.CreateTonkho(v);
        }
      }));
      await Promise.all(existingTonkho
        .filter(sp => !uniqueData.some((item:any) => item.masp === sp.masp))
        .map(sp => this._TonkhoService.updateTonkho({ ...sp, isActive: false }))
      );
  ...
  onPageSizeChange(size: number,menuHienthi:any) {
    if(size>this.Listtonkho().length){
      this.pageSize = this.Listtonkho().length;
      this._snackBar.open(`Số lượng tối đa ${this.Listtonkho().length}`, '', {
        duration: 1000,
        horizontalPosition: "end",
        verticalPosition: "top",
        panelClass: ['snackbar-success'],
  ...
  updateDisplayData() {
    ...
    const pageData = this.Listtonkho().slice(startIndex, endIndex);
