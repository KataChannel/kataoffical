import { AfterViewInit, ChangeDetectionStrategy, Component, computed, effect, inject, TemplateRef, ViewChild } from '@angular/core';
  ...
  export class ListDonhangComponent {
    ...
    constructor() {
      effect(() => {
        ...
        this.totalItems = this.Listdonhang().length;
  ...
  onPageSizeChange(size: number,menuHienthi:any) {
    if(size>this.Listdonhang().length){
      this.pageSize = this.Listdonhang().length;
      this._snackBar.open(`Số lượng tối đa ${this.Listdonhang().length}`, '', {
        duration: 1000,
        horizontalPosition: "end",
        verticalPosition: "top",
        panelClass: ['snackbar-success'],
