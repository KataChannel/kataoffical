...
export class ListCourseComponent implements OnInit {
  ...
  FilterColumns: any[] = JSON.parse(localStorage.getItem('CourseColFilter') || '[]');
  Columns: any[] = [];
  ...
  Listcourse = this._CourseService.ListCourse;
  page = this._CourseService.page;
  pageCount = this._CourseService.pageCount;
  total = this._CourseService.total;
  pageSize = this._CourseService.pageSize;
  courseId = this._CourseService.courseId;
  ...
  async ngOnInit(): Promise<void> {
    this._CourseService.listenCourseUpdates();
    await this._CourseService.getAllCourse(this.pageSize(),true);
    this.displayedColumns = Object.keys(this.ColumnName);
    this.dataSource = new MatTableDataSource(this.Listcourse());
    this.dataSource.sort = this.sort;
  ...
  private initializeColumns(): void {
    ...
    localStorage.setItem('CourseColFilter', JSON.stringify(this.FilterColumns));
  ...
  async getUpdatedCodeIds() {
    await this._CourseService.getUpdatedCodeIds();
  }
  ...
  doFilterHederColumn(event: any, column: any): void {
    this.dataSource.filteredData = this.Listcourse().filter((v: any) => 
  ...
  ResetFilter() {
    this.ListFilter = this.Listcourse();
  }
  ...
  ApplyFilterColum(menu: any) {
    this.dataSource.data = this.Listcourse().filter((v: any) => 
  ...
  DeleteListItem(): void {
    this.EditList.forEach((item: any) => {
      this._CourseService.DeleteCourse(item);
    });
    this.EditList = [];
  ...
  goToDetail(item: any): void {
    this.drawer.open();
    this._CourseService.setCourseId(item.id);
    this._router.navigate(['admin/course', item.id]);
  }
  ...
  async DoImportData(data: any) {
    ...
    const existingCourse = this._CourseService.ListCourse();
    const existingMasp = existingCourse.map((v: any) => v.masp);
    ...
    await Promise.all(uniqueData.map(async (v: any) => {
      const existingItem = existingCourse.find((v1: any) => v1.masp === v.masp);
      if (existingItem) {
        const updatedItem = { ...existingItem, ...v };
        await this._CourseService.updateCourse(updatedItem);
      } else {
        await this._CourseService.CreateCourse(v);
      }
    }));
    await Promise.all(existingCourse
      .filter(sp => !uniqueData.some((item: any) => item.masp === sp.masp))
      .map(sp => this._CourseService.updateCourse({ ...sp, isActive: false }))
    );
  ...
  onPageSizeChange(size: number, menuHienthi: any) {
    ...
    this._CourseService.page.set(1);
    this._CourseService.getAllCourse(size, true);
    menuHienthi.closeMenu();
  }
  onPreviousPage(){
    if (this.page() > 1) {
      this._CourseService.page.set(this.page() - 1);
      this._CourseService.getAllCourse(this.pageSize(), true);
    }
  }
