...
import { environment } from '../../../../environments/environment';
  @Component({
    ...
    styleUrls: ['./listresource.component.scss'],
    imports: [
      MatFormFieldModule,
      MatInputModule,
      MatTableModule,
  ...
  export class ListResourceComponent {
    ...
    Listresource:any = this._ResourceService.ListResource;
    EditList:any=[];
    ...
    constructor() {
      effect(() => {
        ...
        this.totalItems = this.Listresource().length;
    ...
    private initializeColumns(): void {
      ...
      if (this.FilterColumns.length === 0) {
        ...
        localStorage.setItem('ResourceColFilter',JSON.stringify(this.FilterColumns)
        );
      }
    ...
    doFilterHederColumn(event: any, column: any): void {
      this.dataSource.filteredData = this.Listresource().filter((v: any) => v[column].toLowerCase().includes(event.target.value.toLowerCase()));  
    ...
    ResetFilter()
    {
      this.ListFilter = this.Listresource();
      // this.dataSource.data = this.Listresource();
    ...
    {    
      this.dataSource.data = this.Listresource().filter((v: any) => this.ListFilter.some((v1) => v1.id === v.id));
      this.dataSource.paginator = this.paginator;
      this.dataSource.sort = this.sort;
      menu.closeMenu();
    }
    ...
    goToDetail(item: any): void {
      this.drawer.open();
      this._ResourceService.setResourceId(item.id);
      this._router.navigate(['admin/resource', item.id]);
    }
    ...
    async DoImportData(data: any) {
      ...
      const existingResource = this._ResourceService.ListResource();
      const existingMasp  = existingResource.map((v: any) => v.masp);
      ...
      await Promise.all(uniqueData.map(async (v: any) => {
        const existingItem = existingResource.find((v1: any) => v1.masp === v.masp);
        if (existingItem) {
          const updatedItem = { ...existingItem, ...v };
          await this._ResourceService.updateResource(updatedItem);
        } else {
          await this._ResourceService.CreateResource(v);
        }
      }));
      await Promise.all(existingResource
        .filter(sp => !uniqueData.some((item:any) => item.masp === sp.masp))
        .map(sp => this._ResourceService.updateResource({ ...sp, isActive: false }))
      );
  ...
  onPageSizeChange(size: number,menuHienthi:any) {
    if(size>this.Listresource().length){
      this.pageSize = this.Listresource().length;
      this._snackBar.open(`Số lượng tối đa ${this.Listresource().length}`, '', {
        duration: 1000,
        horizontalPosition: "end",
        verticalPosition: "top",
        panelClass: ['snackbar-success'],
  ...
  updateDisplayData() {
    ...
    const pageData = this.Listresource().slice(startIndex, endIndex);
