import { AfterViewInit, ChangeDetectionStrategy, Component, computed, effect, inject, TemplateRef, ViewChild } from '@angular/core';
  ...
  @Component({
    ...
    styleUrls: ['./listquanlyctv.component.scss'],
    imports: [
      MatFormFieldModule,
      MatInputModule,
      MatTableModule,
  ...
  export class ListQuanlyctvComponent {
    ...
    private _QuanlyctvService: QuanlyctvService = inject(QuanlyctvService);
    private _breakpointObserver: BreakpointObserver = inject(BreakpointObserver);
    private _GoogleSheetService: GoogleSheetService = inject(GoogleSheetService);
    private _router: Router = inject(Router);
    private _dialog: MatDialog = inject(MatDialog);
    Listquanlyctv:any = this._QuanlyctvService.ListQuanlyctv;
    EditList:any=[];
    ...
    private initializeColumns(): void {
      ...
      if (this.FilterColumns.length === 0) {
        ...
        localStorage.setItem('QuanlyctvColFilter',JSON.stringify(this.FilterColumns)
        );
      }
    ...
    doFilterHederColumn(event: any, column: any): void {
      this.dataSource.filteredData = this.Listquanlyctv().filter((v: any) => v[column].toLowerCase().includes(event.target.value.toLowerCase()));  
    ...
    ResetFilter()
    {
      this.ListFilter = this.Listquanlyctv();
      // this.dataSource.data = this.Listquanlyctv();
      // this.dataSource.paginator = this.paginator;
      // this.dataSource.sort = this.sort;
    }
    ...
    {    
      this.dataSource.data = this.Listquanlyctv().filter((v: any) => this.ListFilter.some((v1) => v1.id === v.id));
      this.dataSource.paginator = this.paginator;
      this.dataSource.sort = this.sort;
      menu.closeMenu();
    }
    ...
    DeleteListItem(): void {
      this.EditList.forEach((item: any) => {
        this._QuanlyctvService.DeleteQuanlyctv(item);
      });
      this.EditList = [];
    ...
    goToDetail(item: any): void {
      this.drawer.open();
      this._QuanlyctvService.setQuanlyctvId(item.id);
      this._router.navigate(['admin/quanlyctv', item.id]);
    }
    ...
    async DoImportData(data: any) {
      ...
      const existingQuanlyctv = this._QuanlyctvService.ListQuanlyctv();
      const existingMasp  = existingQuanlyctv.map((v: any) => v.masp);
      ...
      await Promise.all(uniqueData.map(async (v: any) => {
        const existingItem = existingQuanlyctv.find((v1: any) => v1.masp === v.masp);
        if (existingItem) {
          const updatedItem = { ...existingItem, ...v };
          await this._QuanlyctvService.updateQuanlyctv(updatedItem);
        } else {
          await this._QuanlyctvService.CreateQuanlyctv(v);
        }
      }));
      await Promise.all(existingQuanlyctv
        .filter(sp => !uniqueData.some((item:any) => item.masp === sp.masp))
        .map(sp => this._QuanlyctvService.updateQuanlyctv({ ...sp, isActive: false }))
      );
  ...
  onPageSizeChange(size: number,menuHienthi:any) {
    if(size>this.Listquanlyctv().length){
      this.pageSize = this.Listquanlyctv().length;
      this._snackBar.open(`Số lượng tối đa ${this.Listquanlyctv().length}`, '', {
        duration: 1000,
        horizontalPosition: "end",
        verticalPosition: "top",
        panelClass: ['snackbar-success'],
  ...
  updateDisplayData() {
    ...
    const pageData = this.Listquanlyctv().slice(startIndex, endIndex);
