import { AfterViewInit, ChangeDetectionStrategy, Component, computed, effect, inject, TemplateRef, ViewChild } from '@angular/core';
  ...
  export class ListUserComponent {
    ...
    doFilterHederColumn(event: any, column: any): void {
      this.dataSource.filteredData = this.Listuser().filter((v: any) => v[column].toLowerCase().includes(event.target.value.toLowerCase()));  
    ...
    {    
      this.dataSource.data = this.Listuser().filter((v: any) => this.ListFilter.some((v1) => v1.id === v.id));
      this.dataSource.paginator = this.paginator;
      this.dataSource.sort = this.sort;
      menu.closeMenu();
    }
    ...
    DeleteListItem(): void {
      this.EditList.forEach((item: any) => {
        this._UserService.DeleteUser(item);
      });
      this.EditList = [];
    ...
    async DoImportData(data: any) {
      ...
      const existingUser = this._UserService.ListUser();
      const existingMasp  = existingUser.map((v: any) => v.masp);
      ...
      await Promise.all(uniqueData.map(async (v: any) => {
        const existingItem = existingUser.find((v1: any) => v1.masp === v.masp);
        if (existingItem) {
          const updatedItem = { ...existingItem, ...v };
          await this._UserService.updateUser(updatedItem);
        } else {
          await this._UserService.CreateUser(v);
        }
      }));
      await Promise.all(existingUser
        .filter(sp => !uniqueData.some((item:any) => item.masp === sp.masp))
        .map(sp => this._UserService.updateUser({ ...sp, isActive: false }))
      );
  ...
  onPageSizeChange(size: number,menuHienthi:any) {
    if(size>this.Listuser().length){
      this.pageSize = this.Listuser().length;
      this._snackBar.open(`Số lượng tối đa ${this.Listuser().length}`, '', {
        duration: 1000,
        horizontalPosition: "end",
        verticalPosition: "top",
        panelClass: ['snackbar-success'],
  ...
  updateDisplayData() {
    ...
    const pageData = this.Listuser().slice(startIndex, endIndex);
