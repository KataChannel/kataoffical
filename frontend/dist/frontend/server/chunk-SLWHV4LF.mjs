...
var GraphqlService = class _GraphqlService {
  ...
  constructor(apollo, platformId, ngZone) {
    this.apollo = apollo;
  ...
  parallelFindAll(modelName, options, batchSize, maxConcurrency) {
    return __async(this, null, function* () {
      const countResult = yield this.findMany(modelName, __spreadProps(__spreadValues({}, options), {
        select: { id: true },
        take: 1e5
        // Ensure we count all existing records within the safe limit
      }));
      const totalCount = countResult.length;
      if (totalCount === 0) {
        return {
  ...
  getNhuCauDatHang(startDate, endDate, forceRefresh = false) {
    return __async(this, null, function* () {
  ...
  saveNhucauNote(sanphamId, content) {
    return __async(this, null, function* () {
      const startTime = Date.now();
      try {
        const result = yield firstValueFrom(this.apollo.mutate({
  ...
  static \u0275fac = function GraphqlService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _GraphqlService)(\u0275\u0275inject(Apollo), \u0275\u0275inject(PLATFORM_ID), \u0275\u0275inject(NgZone));
  };
...
//# sourceMappingURL=chunk-SLWHV4LF.mjs.map
