...
// src/app/shared/uploadfile/uploadfile.service.ts
var UploadService = class _UploadService {
  storage;
  timezoneService;
  constructor(storage, timezoneService) {
    this.storage = storage;
    this.timezoneService = timezoneService;
  }
  DeleteuploadDriver(data) {
    return __async(this, null, function* () {
      console.log(data);
      try {
        const response = yield fetch(environment.APIURL + `/upload/${data.id}`, {
          method: "DELETE",
          headers: {
  ...
  uploadlocal(file, type = "local", folder = "unknown") {
    return __async(this, null, function* () {
      try {
        ...
        const response = yield fetch(environment.APIURL + `/upload/${folder}`, requestOptions);
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
  ...
  uploadDriver(file, type = "googledrive") {
    return __async(this, null, function* () {
      try {
        const formData = new FormData();
        formData.append("file", file);
        const formattedDate = this.timezoneService.nowLocal("DD_MM_YYYY");
        if (type == "googledrive") {
          const response = yield fetch(environment.APIURL + `/upload/googledrive?folderId=1aAvPsWRhjwgDjYulHkHLdEgXlk6694bc`, {
            method: "POST",
            body: formData
          });
          if (!response.ok) {
  ...
  uploadDonhang(file) {
    return __async(this, null, function* () {
      try {
        const response = yield fetch(environment.APIURL + `/upload/image`, {
          method: "POST",
          body: file
        });
        if (!response.ok) {
  ...
  //   }
  // }
  uploadFirebase(image, path = "uploads") {
    return __async(this, null, function* () {
      const storageRef = this.storage.ref(path);
      const uploadTask = this.storage.upload(path, image);
      return new Promise((resolve, reject) => {
        uploadTask.snapshotChanges().pipe(finalize(() => {
          storageRef.getMetadata().subscribe((result) => {
            console.log(result);
          }, (error) => {
            reject(error);
          });
  ...
  static \u0275fac = function UploadService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _UploadService)(\u0275\u0275inject(AngularFireStorage), \u0275\u0275inject(TimezoneService));
  };
...
@firebase/storage-compat/dist/esm/index.esm2017.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-RBMUTUYY.mjs.map
