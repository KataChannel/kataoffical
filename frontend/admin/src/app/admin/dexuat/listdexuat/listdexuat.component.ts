...
@Component({
  selector: 'app-listdexuat',
  templateUrl: './listdexuat.component.html',
  styleUrls: ['./listdexuat.component.scss'],
  imports: [
    MatFormFieldModule,
    MatInputModule,
    MatTableModule,
...
export class ListDexuatComponent {
  ...
  FilterColumns: any[] = JSON.parse(
    localStorage.getItem('DexuatColFilter') || '[]'
  );
  Columns: any[] = [];
  totalItems = 0;
  pageSize = 10;
  ...
  private _DexuatService: DexuatService = inject(DexuatService);
  private _breakpointObserver: BreakpointObserver = inject(BreakpointObserver);
  private _GoogleSheetService: GoogleSheetService = inject(GoogleSheetService);
  private _router: Router = inject(Router);
  private _dialog: MatDialog = inject(MatDialog);
  Listdexuat:any = this._DexuatService.ListDexuat;
  EditList:any=[];
  ...
  constructor() {
    effect(() => {
      this.dataSource.data = this.Listdexuat();
      console.log(this.Listdexuat());
      
      this.totalItems = this.Listdexuat().length;
  ...
  async ngOnInit(): Promise<void> {    
    this._DexuatService.listenDexuatUpdates();
    await this._DexuatService.getAllDexuat();
    this.displayedColumns = Object.keys(this.ColumnName)
    console.log(this.displayedColumns);
    this.updateDisplayData();
    this.dataSource = new MatTableDataSource(this.Listdexuat());
  ...
  private initializeColumns(): void {
    ...
    if (this.FilterColumns.length === 0) {
      ...
      localStorage.setItem('DexuatColFilter',JSON.stringify(this.FilterColumns)
      );
    }
  ...
  doFilterHederColumn(event: any, column: any): void {
    this.dataSource.filteredData = this.Listdexuat().filter((v: any) => v[column].toLowerCase().includes(event.target.value.toLowerCase()));  
  ...
  ResetFilter()
  {
    this.ListFilter = this.Listdexuat();
    // this.dataSource.data = this.Listdexuat();
    // this.dataSource.paginator = this.paginator;
    // this.dataSource.sort = this.sort;
  }
  ...
  {    
    this.dataSource.data = this.Listdexuat().filter((v: any) => this.ListFilter.some((v1) => v1.id === v.id));
    this.dataSource.paginator = this.paginator;
    this.dataSource.sort = this.sort;
    menu.closeMenu();
  }
  ...
  DeleteListItem(): void {
    this.EditList.forEach((item: any) => {
      this._DexuatService.DeleteDexuat(item);
    });
    this.EditList = [];
  ...
  goToDetail(item: any): void {
    this.drawer.open();
    this._DexuatService.setDexuatId(item.id);
    this._router.navigate(['admin/dexuat', item.id]);
  }
  ...
  async DoImportData(data: any) {
    ...
    const existingDexuat = this._DexuatService.ListDexuat();
    const existingMasp  = existingDexuat.map((v: any) => v.masp);
    ...
    await Promise.all(uniqueData.map(async (v: any) => {
      const existingItem = existingDexuat.find((v1: any) => v1.masp === v.masp);
      if (existingItem) {
        const updatedItem = { ...existingItem, ...v };
        await this._DexuatService.updateDexuat(updatedItem);
      } else {
        await this._DexuatService.CreateDexuat(v);
      }
    }));
    await Promise.all(existingDexuat
      .filter(sp => !uniqueData.some((item:any) => item.masp === sp.masp))
      .map(sp => this._DexuatService.updateDexuat({ ...sp, isActive: false }))
    );
...
onPageSizeChange(size: number,menuHienthi:any) {
  if(size>this.Listdexuat().length){
    this.pageSize = this.Listdexuat().length;
    this._snackBar.open(`Số lượng tối đa ${this.Listdexuat().length}`, '', {
      duration: 1000,
      horizontalPosition: "end",
      verticalPosition: "top",
      panelClass: ['snackbar-success'],
...
updateDisplayData() {
  ...
  const pageData = this.Listdexuat().slice(startIndex, endIndex);
