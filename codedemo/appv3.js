...
const MAP_SIZE = 5; // Cập nhật: Kích thước bản đồ 5x5
...
const initializeGame = () => {
  ...
  for (let r = 0; r < MAP_SIZE; r++) {
    for (let c = 0; c < MAP_SIZE; c++) {
      if (initialMap[r][c].hiddenType === woodForestType) { // Check hiddenType
  ...
  if (!woodForestTile) {
      if (availablePositions.length > 0) {
          ...
          console.error("No available positions for Wood Forest! This indicates a problem with map generation.");
      }
  }
...
const loadGame = () => {
  try {
    ...
    if (loadedState.sources && loadedState.sources.wood_forest && loadedState.sources.wood_forest.lastHarvestTime > 0) {
...
const discoverTile = (gameState, row, col) => {
  ...
  if (tile.hiddenType === 'empty') {
    ...
    if (!costType) { // Trường hợp dự phòng nếu không có quan hệ sinh cụ thể
  ...
  if (newResources[costType] < costAmount) {
    return { success: false, message: `Không đủ tài nguyên để khám phá ô này. Cần ${costAmount} ${costType}.` };
  }
...
const activateTile = (gameState, row, col) => {
  ...
  if (gameState.sources[tile.type]) { 
    ...
    if (newResources[generatingResource] < ACTIVATION_COST_AMOUNT) {
      return { success: false, message: `Không đủ tài nguyên để kích hoạt. Cần ${ACTIVATION_COST_AMOUNT} ${generatingResource}.` };
    }
    ...
    message = `Đã kích hoạt nguồn ${tile.type.replace('_', ' ').replace('mine', 'Mỏ').replace('forest', 'Rừng').replace('spring', 'Suối').replace('forge', 'Lò').replace('field', 'Đất')}! Mất ${ACTIVATION_COST_AMOUNT} ${generatingResource}.`;
  } else if (gameState.spiritBeasts[tile.type]) { // Nếu là một linh thú
...
const harvestSource = (gameState, sourceKey) => {
  ...
  return {
    ...
    message: `Đã thu hoạch ${actualYield} ${primaryResourceType} và ${generatedYield} ${generatedResourceType} từ ${source.type.replace('_', ' ').replace('mine', 'Mỏ').replace('forest', 'Rừng').replace('spring', 'Suối').replace('forge', 'Lò').replace('field', 'Đất')}!`,
...
const ResourceItem = ({ type, amount, icon, lastChange }) => {
  ...
  return (
    <div className="relative flex items-center space-x-2 p-1 bg-gray-600 rounded-md overflow-hidden">
...
// ResourcePanel.js
const ResourcePanel = ({ resources, animatedResourceChanges }) => {
  return (
...
const Tile = ({ tile, onClick }) => {
  ...
  let tileBgClass = 'bg-gray-900 hover:bg-gray-700'; // Mặc định cho ô chưa khám phá
  ...
  if (isDiscovered) {
    if (isActive && isSpecialTile) {
      ...
      tileBgClass = ELEMENT_TILE_BG_COLORS[elementType] || 'bg-gray-800'; // Màu nền dựa trên thuộc tính, hoặc xám đậm nếu không xác định
    }
  }
    ...
    <div className="p-4 bg-gray-700 rounded-lg shadow-inner">
      ...
      <div className="flex flex-col gap-3">
        {sortedSources.map(source => {
          ...
          const canHarvest = cooldownRemaining === 0 && source.isActive && (source.type === 'wood_forest' || canHarvestOtherSources);
...
const ActivityLog = ({ logs }) => {
  ...
  return (
    ...
    <div className="w-full lg:w-1/4 bg-gray-800 p-6 rounded-xl shadow-2xl border border-gray-700 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar"> {/* Cập nhật các lớp ở đây */}
        {logs.map((log, index) => (
    ...
    setGameState(prevGameState => {
      ...
      for (const sourceKey in currentGameState.sources) {
        ...
        const canHarvest = source.isActive &&
                           ...
                           (source.type === 'wood_forest' || currentGameState.canHarvestOtherSources);
        ...
        if (canHarvest) {
          ...
          if (harvestResult.success) {
            ...
            for (const resType in harvestResult.yieldedAmounts) {
              totalYieldedAmounts[resType] = (totalYieldedAmounts[resType] || 0) + harvestResult.yieldedAmounts[resType];
            }
          }
        }
      }
      ...
      <div className="flex flex-col lg:flex-row w-full max-w-7xl gap-6">
        ...
        <div className="w-full lg:w-1/4 flex flex-col gap-6">
          <div className="bg-gray-800 p-6 rounded-xl shadow-2xl border border-gray-700">
            <ResourcePanel resources={gameState.resources} animatedResourceChanges={animatedResourceChanges} />
            <button
