...
async function performBaselineChot07June() {
  const prisma = new PrismaClient({ datasources: { postgres: { url: DB_URL } } });

  try {
    const filePath = '/home/kata/Coding/rausachfinal/doisoat/Ton-Huy 07-6.xlsx';
    console.log(`--- Processing Baseline & Cleanup (07-06-2026) on rausachfinal DB ---`);
    ...
    // Tomorrow is 2026-06-08
    const tomorrowStart = new Date('2026-06-08T00:00:00+07:00');
    const baselineTime = new Date('2026-06-07T23:59:59+07:00');
    ...
    // 3. Find old chotkho and vouchers dynamically to clean up (same day 07/06)
    const oldChotkhos = await prisma.chotkho.findMany({
      where: {
        ngaychot: {
          gte: new Date('2026-06-07T00:00:00+07:00'),
          lte: new Date('2026-06-07T23:59:59+07:00')
        }
      }
    });
    ...
    const newChotkhoCodeId = `CHOTKHO_BASELINE_0706_${Date.now()}`;
    ...
    // Get all remaining 'dadat' orders to calculate correct pending sl (deliveries on 08/06 or later)
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const chotkhoMaster = await tx.chotkho.create({
        data: {
          ...
          title: `Chốt kho Base Line 07-06-2026`,
          ghichu: 'Chốt kho Baseline chuẩn theo file Ton-Huy 07-6.xlsx - Cập nhật số tồn chuẩn và dọn dẹp đơn cũ',
          khoId: KHO_TONG_ID,
    ...
    console.log('\n--- BASELINE 07-06 RECONCILIATION COMPLETE ---');
