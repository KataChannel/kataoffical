...
export class PerformanceLogger {
  ...
  private static recordMetric(metric: PerformanceMetric) {
    ...
    if (this.performanceLogService) {
      this.performanceLogService.saveMetric(metric).catch((error: any) => {
        this.logger.error(
          `Failed to save metric to database: ${error.message}`,
        );
      });
    }
  }
  ...
  static getStatistics() {
    ...
    const last1Hour = this.metrics.filter(
    ...
    return {
      ...
      last5Minutes: {
        ...
        avgDuration:
          last5Minutes.length > 0
            ? last5Minutes.reduce((sum, m) => sum + m.duration, 0) /
              last5Minutes.length
            : 0,
        successRate:
          last5Minutes.length > 0
            ? (last5Minutes.filter((m) => m.success).length /
                last5Minutes.length) *
              100
            : 100,
      },
      last1Hour: {
        ...
        avgDuration:
          last1Hour.length > 0
            ? last1Hour.reduce((sum, m) => sum + m.duration, 0) /
              last1Hour.length
            : 0,
        successRate:
          last1Hour.length > 0
