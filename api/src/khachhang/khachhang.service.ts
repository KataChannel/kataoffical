...
export class KhachhangService {
  ...
  async generateMakh(loaikh: string): Promise<string> {
    return PerformanceLogger.logAsync(
      'KH_generateMakh',
      async () => {
  ...
  async create(data: any) {
    ...
    if (!banggia) {
      console.warn(
        `Bảng giá với mã ${data.mabanggia} không tồn tại, bỏ qua thiết lập bảng giá`,
      );
    } else {
  ...
  async import(data: any[]) {
    ...
    for (const customer of data) {
      try {
        ...
        if (!banggia) {
          console.warn(
            `Bảng giá với mã ${customer.mabanggia} không tồn tại, bỏ qua khách hàng này`,
          );
          customer.banggiaId = null; // Không gán banggiaId nếu không tìm thấy bảng giá
          notifications.push(
            `Bỏ qua khách hàng với makh ${customer.makh} do không tìm thấy bảng giá`,
          );
          continue;
        } else {
        ...
        if (existingCustomer) {
          ...
          notifications.push(
            `Cập nhật khách hàng thành công: ${processedCustomer.makh}`,
          );
        } else {
          processedCustomer = await this.create(customer);
          notifications.push(
            `Tạo mới khách hàng thành công: ${processedCustomer.makh}`,
          );
        }
      } catch (error) {
        console.error(
        ...
        notifications.push(
          `Lỗi xử lý khách hàng với makh ${customer.makh}: ${error.message}`,
        );
      }
    }
    ...
    if (data.banggiaId) {
      ...
      if (!banggia) {
        throw new NotFoundException(
          `Không tìm thấy bảng giá với ID: ${data.banggiaId}`,
        );
      }
    }
