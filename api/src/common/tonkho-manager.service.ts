  ...
  async validateTonkhoConsistency(): Promise<TonkhoValidationResult> {
    ...
    try {
      ...
      for (const tonkho of tonkhoData) {
        const pendingOut = tonkho.sanpham.Donhangsanpham.reduce(
          (sum, item) => sum + Number(item.slgiao || 0),
          0,
        );

        const pendingIn = tonkho.sanpham.Dathangsanpham.reduce(
          (sum, item) => sum + Number(item.slgiao || 0),
          0,
        );
        ...
        const chogiaoActual =
          Math.round(Number(tonkho.slchogiao) * 1000) / 1000;
        ...
        const chonhapActual =
          Math.round(Number(tonkho.slchonhap) * 1000) / 1000;
        ...
        if (Number(tonkho.slton) < 0) {
          warnings.push(
            `TonKho ${tonkho.sanpham.masp}: Negative inventory ${tonkho.slton}`,
          );
        }

        // Check if available inventory can fulfill pending orders
        const availableInventory =
          Number(tonkho.slton) - Number(tonkho.slchogiao);
  ...
  async recalculateTonkho(
    ...
    try {
      ...
      for (const sanpham of sanphams) {
        const pendingOut = sanpham.Donhangsanpham.reduce(
          (sum, item) => sum + Number(item.slgiao || 0),
          0,
        );

        const pendingIn = sanpham.Dathangsanpham.reduce(
          (sum, item) => sum + Number(item.slgiao || 0),
          0,
        );
