...
export class DonhangController {
  ...
  @UseGuards(JwtAuthGuard)
  @Audit({
    entity: 'Import Donhang Cu Confirmed',
    action: AuditAction.CREATE,
    includeResponse: true,
  })
  @CacheInvalidate(['donhang:*'])
  ImportDonhangOldConfirmed(
    @Body() data: { pendingOrders: any[]; userChoice: 'proceed' | 'skip' },
  ) {
    return this.donhangService.ImportDonhangOldConfirmed(
      data.pendingOrders,
      data.userChoice,
    );
  }
  ...
  @Post('searchfield')
  @UseGuards(JwtAuthGuard)
  async searchfield(@Body() params: any) {
    return this.donhangService.searchfield(params);
  }

  @Post('xac-nhan-giao-thuc-te/:id')
  @UseGuards(JwtAuthGuard)
  async xacNhanGiaoThucTe(
    @Param('id') id: string,
    @Body() data: { ghichu?: string },
  ) {
    return this.donhangService.xacNhanGiaoThucTe(id, data);
  }
  ...
  async doiChieu(
    ...
    return this.donhangService.doiChieu(id, data);
  }
  // @Get()
  // findAll() {
  ...
  @UseGuards(JwtAuthGuard)
  @Audit({
    entity: 'Update Phieugiao',
    action: AuditAction.UPDATE,
    includeResponse: true,
  })
  async updatePhieugiao(
    @Param('id') id: string,
    @Body() updateDonhangDto: any,
  ) {
    const result = await this.donhangService.updatePhieugiao(
      id,
      updateDonhangDto,
    );
    return result;
  ...
  @UseGuards(JwtAuthGuard)
  @Audit({
    entity: 'Manual Auto Complete Orders',
    action: AuditAction.UPDATE,
    includeResponse: true,
  })
  ...
  @UseGuards(JwtAuthGuard)
  @Audit({
    entity: 'Complete Pending Deliveries',
    action: AuditAction.UPDATE,
    includeResponse: true,
  })
  async completePendingDeliveries(@Param('sanphamId') sanphamId: string) {
    try {
      const result =
        await this.donhangService.completePendingDeliveriesForProduct(
          sanphamId,
        );
      return result;
    } catch (error) {
  ...
  @ApiOperation({
    summary: 'Verify order prices against current banggia prices',
  })
  ...
  async getPriceAtDate(
    ...
    return this.donhangService.getPriceAtDate(
      banggiaId,
      sanphamId,
      new Date(date),
    );
  ...
  async generateConfirmToken(@Param('id') id: string) {
    ...
    return {
      ...
      message:
        'Token đã được tạo. Gửi link này cho khách hàng để xác nhận đơn hàng.',
    };
  }
}
