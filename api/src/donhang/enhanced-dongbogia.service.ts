...
export class EnhancedDongbogiaService {
  ...
  async dongbogiaEnhanced(listdonhang: any[]) {
    ...
    console.log(
      `📊 Processing ${listdonhang.length} orders in batches of ${batchSize}`,
    );
    ...
    for (let i = 0; i < listdonhang.length; i += batchSize) {
      ...
      while (!batchSuccess && retryCount <= maxRetries) {
        try {
          ...
          const batchResult = await this.serverStability.safeTransaction(
            async (prisma) => {
              let updatedCount = 0;
              let errorCount = 0;
                ...
                try {
                  ...
                  const donhang = await Promise.race([
                    ...
                    new Promise((_, reject) =>
                      setTimeout(
                        () => reject(new Error('Order query timeout')),
                        5000,
                      ),
                    ),
                  ]);
                  ...
                  if (
                    ...
                    console.warn(
                      `⚠️ Order ${donhang.madonhang} missing price list or products`,
                    );
                    errorCount++;
                    continue;
                  }
                  ...
                  console.log(
                    `🔄 Updating prices for order ${donhang.madonhang} (${donhang.sanpham.length} products)`,
                  );
                  ...
                  for (const donhangSanpham of donhang.sanpham) {
                    try {
                      ...
                      if (giaSanpham && Number(giaSanpham.giaban) > 0) {
                        ...
                        console.warn(
                          `⚠️ No valid price found for product ${donhangSanpham.sanpham?.title || donhangSanpham.idSP}`,
                        );
                      ...
                      console.log(
                        `✅ Updated product ${donhangSanpham.sanpham?.title} - Price: ${giaban} (from ${giaSource})`,
                      );
                    } catch (productError) {
                      console.error(
                        `❌ Error updating product ${donhangSanpham.idSP}:`,
                  ...
                  if (hasUpdates && productUpdates.length > 0) {
                    ...
                    for (
                      let pi = 0;
                      pi < productUpdates.length;
                      pi += subBatchSize
                    ) {
                      const productBatch = productUpdates.slice(
                        pi,
                        pi + subBatchSize,
                      );

                      await Promise.all(
                        productBatch.map((update) =>
                    ...
                    console.log(
                      `✅ Order ${donhang.madonhang}: Updated totals - Base: ${tongchua}, VAT: ${tongvat}, Total: ${tongtien}`,
                    );
                  }
                  ...
                  console.error(
                    `❌ Error processing order ${donhangId}:`,
                    orderError.message,
                  );
                  errorCount++;
          ...
          console.log(
            `✅ Batch ${batchNumber} completed: ${batchResult.updatedCount} success, ${batchResult.errorCount} errors`,
          );
        } catch (batchError) {
          retryCount++;
          console.error(
            `❌ Batch ${batchNumber} failed (attempt ${retryCount}):`,
            batchError.message,
          );

          if (retryCount <= maxRetries) {
            console.log(
              `⏳ Retrying batch ${batchNumber} in ${retryCount * 2} seconds...`,
            );
            ...
            console.error(
              `💀 Batch ${batchNumber} failed after ${maxRetries} retries, skipping`,
            );
    ...
    const successRate = Math.round(
      (totalUpdatedCount / listdonhang.length) * 100,
    );
    console.log(
      `🎯 Price sync completed: ${totalUpdatedCount}/${listdonhang.length} orders (${successRate}%)`,
    );
