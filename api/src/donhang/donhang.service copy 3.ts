...
//     return this.createCongnoExcelFile(result || [], params, donhangs);
//   }

//   /**
//    * Create Excel file for Congno with grouped data by customer
//    */
//   private async createCongnoExcelFile(data: any[], params: any, donhangs: any[] = []) {
...
//         throw new Error(`Invalid column number: ${columnNumber}. Excel supports columns from 1 to 16384.`);
//       }
//       let letter = '';
...
//     // ✅ THÊM HEADER THEO YÊU CẦU
    
//     // Ô A1: Logo/Văn bản "TRAN GIA" với màu đỏ và nền hình quả ớt
...
//       type: 'pattern',
//       pattern: 'solid',
//       fgColor: { argb: 'FFFFCC00' } // Màu nền vàng-cam (giống quả ớt)
//     };
...
//     // Ô A2: "SAFETY FOODS FOR BETTER HEALTH" với màu xanh lá và font in đậm
//     const sloganCell = worksheet.getCell('A2');
//     sloganCell.value = 'SAFETY FOODS FOR BETTER HEALTH';
//     sloganCell.font = { 
//       bold: true, 
//       size: 12, 
//       color: { argb: 'FF00AA00' } // Màu xanh lá
//     };
//     sloganCell.alignment = { horizontal: 'left', vertical: 'middle' };

//     // Ô A4: Tiêu đề "BẢNG ĐỐI CHIẾU CÔNG NỢ"
//     const titleCell = worksheet.getCell('A4');
//     titleCell.value = 'BẢNG ĐỐI CHIẾU CÔNG NỢ';
//     titleCell.font = { 
//       bold: true, 
//       size: 14, 
//       color: { argb: 'FFFF0000' } // Màu đỏ
...
//     // Lấy thông tin khách hàng đầu tiên để hiển thị (nếu có)
//     const firstCustomer = data.length > 0 ? data[0] : null;
//     const tenkhachhang = firstCustomer?.tenkhachhang || '[Tên khách hàng]';
    
//     // Lấy thông tin khách hàng chi tiết từ donhangs
//     let customerInfo = { name: tenkhachhang, diachi: '[Địa chỉ khách hàng]' };
//     if (donhangs.length > 0 && donhangs[0].khachhang) {
//       customerInfo = {
//         name: donhangs[0].khachhang.name || tenkhachhang,
//         diachi: donhangs[0].khachhang.diachi || '[Địa chỉ khách hàng]'
//       };
//     }
    
//     // Ô A6: "Tên Khách hàng: {{tenkhachhang}}"
//     const customerNameCell = worksheet.getCell('A6');
//     customerNameCell.value = `Tên Khách hàng: ${customerInfo.name}`;
...
//     addressCell.value = `Địa chỉ: ${customerInfo.diachi}`;
...
//     // Ô A8: "Người liên hệ:" và Ô B8: "Email:"
//     const contactCell = worksheet.getCell('A8');
//     contactCell.value = 'Người liên hệ:';
//     contactCell.font = { bold: true };
...
//     const emailCell = worksheet.getCell('B8');
...
//     // ✅ ĐỊNH NGHĨA CỘT DỮ LIỆU (BẮT ĐẦU TỪ HÀNG 10)
...
//     // Merge một số ô để layout đẹp hơn (sau khi đã định nghĩa columns)
//     const totalColumns = columns.length; // 16 columns  
//     const lastColumnLetter = getColumnLetter(totalColumns); // Sử dụng helper function
    
//     // Helper function to safely get column index (defined after columns)
//     const getColumnIndex = (key: string): number => {
//       const index = columns.findIndex(c => c.key === key) + 1;
//       if (index === 0) {
//         console.warn(`Column with key '${key}' not found`);
//         return 1; // Default to column A
//       }
//       return index;
//     };
    
//     worksheet.mergeCells(`A4:${lastColumnLetter}4`); // Merge tiêu đề qua tất cả các cột
//     worksheet.mergeCells('A6:H6'); // Merge tên khách hàng
//     worksheet.mergeCells('A7:H7'); // Merge địa chỉ
...
//     // Tạo header row cho dữ liệu (hàng 10)
//     const dataHeaderRow = worksheet.getRow(dataStartRow);
//     // ✅ FIX: Set header values với null ở index 0 để khớp với data rows
//     dataHeaderRow.values = [null, ...columns.map(col => col.header)];
...
//     let currentRow = dataStartRow + 1; // Bắt đầu từ hàng 11
...
//             // ✅ FIX: Sử dụng object mapping thay vì array để đảm bảo đúng cột
//             const rowData = {
...
//             // Set values theo đúng thứ tự columns
//             const rowValues = columns.map(col => rowData[col.key]);
//             row.values = [null, ...rowValues]; // null for index 0, data from index 1

//             // ✅ FIX: Format number columns theo column key thay vì hardcode index
//             const numberColumns = ['soluong', 'dongia', 'thanhtientruocvat', 'dongiavathoadon', 'thanhtiensauvat', 'tongtiensauvat', 'tongcong'];
...
//             // ✅ FIX: VAT column formatting
//             const vatColIndex = getColumnIndex('vat');
//             const vatCell = row.getCell(vatColIndex);
//             if (vatCell.value !== null && vatCell.value !== undefined) {
//               vatCell.numFmt = '0.00%';
...
//             const makhachhangColIndex = getColumnIndex('makhachhang');
//             const tenkhachhangColIndex = getColumnIndex('tenkhachhang');
//             const madonhangColIndex = getColumnIndex('madonhang');
            
//             // Merge makhachhang theo madonhang
//             mergeRanges.push({
//               range: `${getColumnLetter(makhachhangColIndex)}${orderStartRow}:${getColumnLetter(makhachhangColIndex)}${orderEndRow}`,
//               value: orderItems[0].makhachhang || ''
//             });
            
//             // Merge tenkhachhang theo madonhang
//             mergeRanges.push({
//               range: `${getColumnLetter(tenkhachhangColIndex)}${orderStartRow}:${getColumnLetter(tenkhachhangColIndex)}${orderEndRow}`,
//               value: orderItems[0].tenkhachhang || ''
//             });
            
//             // Merge madonhang theo madonhang (giống logic trước đó)
//             mergeRanges.push({
//               range: `${getColumnLetter(madonhangColIndex)}${orderStartRow}:${getColumnLetter(madonhangColIndex)}${orderEndRow}`,
...
//           const ngaygiaoColIndex = getColumnIndex('ngaygiao');
//           mergeRanges.push({
//             range: `${getColumnLetter(ngaygiaoColIndex)}${dateStartRow}:${getColumnLetter(ngaygiaoColIndex)}${dateEndRow}`,
...
//           const tongtiensauvatColIndex = getColumnIndex('tongtiensauvat');
//           mergeRanges.push({
//             range: `${getColumnLetter(tongtiensauvatColIndex)}${dateStartRow}:${getColumnLetter(tongtiensauvatColIndex)}${dateEndRow}`,
...
//         const tongcongColIndex = getColumnIndex('tongcong');
//         mergeRanges.push({
//           range: `${getColumnLetter(tongcongColIndex)}${customerStartRow}:${getColumnLetter(tongcongColIndex)}${customerEndRow}`,
//           value: customerData.tongtiensauvat  // Tổng cộng của cả khách hàng
//         });
//       }
//     }

//     // Apply merge ranges và thiết lập styling
...
//       // Thêm màu nền khác nhau cho các loại merge
//       if (merge.range.includes('A')) { // Cột khách hàng
//         cell.fill = {
//           type: 'pattern',
//           pattern: 'solid',
//           fgColor: { argb: 'FFE6F3FF' } // Xanh nhạt
//         };
//       } else if (merge.range.includes(getColumnLetter(getColumnIndex('ngaygiao')))) { // Cột ngày giao
//         cell.fill = {
//           type: 'pattern',
//           pattern: 'solid',
//           fgColor: { argb: 'FFF0F8E6' } // Xanh lá nhạt
//         };
//       } else { // Các cột số tiền
//         cell.fill = {
//           type: 'pattern',
//           pattern: 'solid',
//           fgColor: { argb: 'FFFFF2E6' } // Cam nhạt
//         };
//       }
//     });

//     // Add borders và styling cho tất cả cells
...
//         // Styling cho header rows
//         if (rowNumber <= 7) { // Company header area
//           cell.font = { bold: true };
//           if (rowNumber === 3) { // Company name
//             cell.font = { bold: true, size: 16 };
//           }
//         }
        
//         // Styling cho data header
//         if (rowNumber === 8) {
//           cell.fill = {
//             type: 'pattern',
//             pattern: 'solid',
...
//     columns.forEach((col, index) => {
//       const columnIndex = index + 2; // +2 vì bắt đầu từ cột B
//       const column = worksheet.getColumn(columnIndex);
      
//       // Tính toán width dựa trên content
//       let maxWidth = col.header.length;
//       worksheet.eachRow((row, rowNumber) => {
//         if (rowNumber > 8) { // Chỉ check data rows
...
//       // Set width với min/max constraints
//       column.width = Math.min(Math.max(maxWidth + 2, 10), 30);
//     });
    
//     // Set width cho cột khách hàng (cột A)
