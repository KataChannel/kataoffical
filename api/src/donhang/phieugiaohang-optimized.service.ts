...
export class PhieuGiaoHangOptimizedService {
  ...
  async updatePhieugiaoOptimized(id: string, data: any) {
    ...
    try {
      ...
      this.logger.error(
        `updatePhieugiao failed after ${duration}ms for order ${id}:`,
        error,
      );
      throw error;
    } finally {
      ...
      if (duration > 5000) {
        this.logger.warn(
          `SLOW OPERATION: updatePhieugiao took ${duration}ms for order ${id}`,
        );
      }
    }
  }
  ...
  async updateBulkOptimized(ids: string[], status: string) {
    ...
    this.logger.log(
      `Starting bulk update for ${ids.length} orders with status: ${status}`,
    );
    ...
    for (let i = 0; i < ids.length; i += this.BATCH_SIZE) {
      ...
      try {
        ...
        this.logger.log(
          `Batch ${Math.floor(i / this.BATCH_SIZE) + 1} completed: ${success}/${batch.length} successful in ${batchDuration}ms`,
        );
        ...
        this.logger.error(
          `Batch ${Math.floor(i / this.BATCH_SIZE) + 1} failed:`,
          error,
        );
    ...
    this.logger.log(
      `Bulk update completed: ${totalSuccess}/${ids.length} successful in ${totalDuration}ms`,
    );
