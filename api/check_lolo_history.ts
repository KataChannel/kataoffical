...
async function main() {
  ...
  try {
    ...
    // 1. Get all ChotKho details for this product in June 2026
    ...
    // 2. Get all PhieuKhoSanpham for this product in June 2026 grouped by day
    ...
    const dailySummary: { [dateStr: string]: { nhap: number, xuat: number } } = {};
    for (const pk of phieuKhos) {
      if (!pk.phieuKho.ngay) continue;
