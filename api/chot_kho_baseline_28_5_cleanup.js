...
async function performBaselineChot28May() {
  const prisma = new PrismaClient({ datasources: { postgres: { url: DB_URL } } });

  try {
    const filePath = '/home/kata/Coding/rausachfinal/doisoat/Ton-Huy 28-5.xlsx';
    console.log(`--- Processing Baseline & Cleanup (28-05-2026) on rausachfinal DB: ${filePath} ---`);
    ...
    // Tomorrow is 2026-05-29
    const tomorrowStart = new Date('2026-05-29T00:00:00+07:00');
    const baselineTime = new Date('2026-05-28T23:59:59+07:00');
    ...
    const oldChotkhos = await prisma.chotkho.findMany({
      where: {
        ngaychot: {
          gte: new Date('2026-05-28T00:00:00+07:00'),
          lte: new Date('2026-05-28T23:59:59+07:00')
        }
      }
    });
    ...
    const newChotkhoCodeId = `CHOTKHO_BASELINE_2805_${Date.now()}`;
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const chotkhoMaster = await tx.chotkho.create({
        data: {
          ...
          title: `Chốt kho Base Line 28-05-2026`,
          ghichu: 'Chốt kho Baseline chuẩn theo file Ton-Huy 28-5.xlsx - Khắc phục lệch giờ các đơn nhập tự động phát sinh sau chốt',
          khoId: KHO_TONG_ID,
    ...
    console.log('\n--- BASELINE 28-05 RECONCILIATION COMPLETE ---');
