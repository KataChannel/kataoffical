...
async function performBaselineChot11May() {
  try {
    const filePath = '/home/kata/Coding/rausachfinal/doisoat/Ton-Huy 11-5.xlsx';
    ...
    // 2. Identify Backlogged Orders (Status 'dadat' created before end of 11/05)
    const baselineTime = new Date('2026-05-11T23:59:59');
    ...
    const chotkhoMaster = await prisma.chotkho.create({
      data: {
        ...
        title: `Chốt kho Base Line 11-05-2026 (FULL CLEANUP)`,
        ghichu: 'Chốt kho Baseline + Dọn dẹp tồn đọng slchogiao/slchonhap + Chuyển trạng thái choxuly',
        khoId: KHO_TONG_ID,
        codeId: `CHOTKHO_CLEANUP_1105_${Date.now()}`,
        isActive: true
      }
    });
    ...
    await prisma.$transaction(async (tx) => {
      ...
      for (const sp of allProducts) {
        ...
        // Create Chotkhodetail using relation connect
        await tx.chotkhodetail.create({
          data: {
            chotkho: { connect: { id: chotkhoMaster.id } },
            sanpham: { connect: { id: sp.id } },
