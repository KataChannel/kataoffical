...
async function importData() {
  const mathangs = await prisma.mathang.findMany();
  for (const mathang of mathangs) {
    if (mathang.title) {
      try {
        await prisma.mathang.update({
          where: { id: mathang.id },
          data: { title2: mathang.title },
        });
        console.log(`✅ Updated title for mathang with ID ${mathang.id}`);
      } catch (error) {
        console.error(`⚠️ Error updating title for mathang with ID ${mathang.id}:`, error.message);
      }
    }
  }

  console.log('✅ Completed updating mathang titles!');
