...
async function performBaselineChot31May() {
  const prisma = new PrismaClient({ datasources: { postgres: { url: DB_URL } } });

  try {
    const filePath = '/home/kata/Coding/rausachfinal/doisoat/Ton-Huy 31-5.xlsx';
    console.log(`--- Processing Baseline & Cleanup (31-05-2026) on rausachfinal DB: ${filePath} ---`);
    ...
    // Tomorrow is 2026-06-01
    const tomorrowStart = new Date('2026-06-01T00:00:00+07:00');
    const baselineTime = new Date('2026-05-31T23:59:59+07:00');
    ...
    console.log(`Found ${oldChotkhoIds.length} old chotkho record(s) to delete:`, oldChotkhoIds.map(c => c.title || c.id));

    const oldVouchers = await prisma.phieuKho.findMany({
      where: {
        maphieu: {
          contains: 'DOISOAT-31052026'
        }
      }
    });
    ...
    const newChotkhoCodeId = `CHOTKHO_BASELINE_3105_${Date.now()}`;
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const chotkhoMaster = await tx.chotkho.create({
        data: {
          ...
          title: `Chốt kho Base Line 31-05-2026`,
          ghichu: 'Chốt kho Baseline chuẩn theo file Ton-Huy 31-5.xlsx - Khắc phục lệch giờ các đơn nhập tự động phát sinh sau chốt',
          khoId: KHO_TONG_ID,
    ...
    console.log('\n--- BASELINE 31-05 RECONCILIATION COMPLETE ---');
    console.log(`- Old Chotkhos deleted: ${oldChotkhoIds.length}`);
    console.log(`- Old faulty vouchers deleted: ${oldVoucherIds.length}`);
