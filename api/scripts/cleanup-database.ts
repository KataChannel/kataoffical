...
async function cleanupLogTables(dryRun: boolean = false): Promise<CleanupResult[]> {
  const results: CleanupResult[] = [];
  const now = new Date();

  // Cấu hình: bảng → số ngày giữ lại
  const cleanupConfig = [
    { 
      name: 'AuditLog',
      keepDays: 30,
      deleteFunc: async (beforeDate: Date) => {
        if (dryRun) return { count: await prisma.auditLog.count({ where: { createdAt: { lt: beforeDate } } }) };
    ...
    },
    {
      ...
      keepDays: 7,
      deleteFunc: async (beforeDate: Date) => {
        if (dryRun) return { count: await prisma.performanceLog.count({ where: { timestamp: { lt: beforeDate } } }) };
    ...
    },
    {
      ...
      keepDays: 14,
      deleteFunc: async (beforeDate: Date) => {
        if (dryRun) return { count: await prisma.errorLog.count({ where: { createdAt: { lt: beforeDate } } }) };
    ...
    },
    {
      ...
      keepDays: 30,
      deleteFunc: async (beforeDate: Date) => {
        if (dryRun) return { count: await prisma.importHistory.count({ where: { createdAt: { lt: beforeDate } } }) };
    ...
    },
    {
      ...
      keepDays: 30,
      deleteFunc: async (beforeDate: Date) => {
        if (dryRun) return { count: await prisma.chatAIMessage.count({ where: { createdAt: { lt: beforeDate } } }) };
    ...
    },
    {
      ...
      keepDays: 30,
      deleteFunc: async (beforeDate: Date) => {
        if (dryRun) return { count: await prisma.chatAIHistory.count({ where: { createdAt: { lt: beforeDate } } }) };
    ...
    },
    {
      ...
      keepDays: 90,
      deleteFunc: async (beforeDate: Date) => {
        if (dryRun) return { count: await prisma.banggiasanphamHistory.count({ where: { changedAt: { lt: beforeDate } } }) };
    ...
    },
    {
      ...
      keepDays: 90,
      deleteFunc: async (beforeDate: Date) => {
        if (dryRun) return { count: await prisma.donhangPriceAudit.count({ where: { createdAt: { lt: beforeDate } } }) };
  ...
  for (const config of cleanupConfig) {
    ...
    try {
      ...
      // Ước tính dung lượng (trung bình ~1KB/row cho log tables)
