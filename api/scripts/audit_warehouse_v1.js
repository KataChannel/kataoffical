...
async function auditWarehouseStatus() {
  try {
    const lastChotTime = new Date('2026-05-08T17:00:00Z');
    ...
    // 6. Check specific missing slips reported
    const missingSlipsReported = ['TG-AA37654', 'TG-AA41053'];
    const ordersToCheck = await prisma.donhang.findMany({
      where: {
        madonhang: { in: missingSlipsReported }
      },
      include: {
        PhieuKho: true
      }
    ...
    const pendingAnomalies = await prisma.tonKho.findMany({
      where: {
        slchogiao: { gt: 100 } // Example threshold for "large"
      },
      include: {
    ...
    console.log(`4. Products with actual movement (via Slips): ${productsWithMovement.length}`);
    console.log(`5. Negative Stock Items: ${negativeStockItems.length}`);
    if (negativeStockItems.length > 0) {
      console.log('   Top 5 negative items:');
      negativeStockItems.slice(0, 5).forEach(item => {
        console.log(`   - ${item.sanpham.title} (${item.sanpham.masp}): ${item.slton}`);
      });
    }

    console.log('\n6. Checking reported missing slips:');
    ordersToCheck.forEach(o => {
      console.log(`   - Order ${o.madonhang}: Status ${o.status}, Slips count: ${o.PhieuKho.length}`);
    });

    console.log('\n7. Top products with high "Pending Delivery" (slchogiao):');
