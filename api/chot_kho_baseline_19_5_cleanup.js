...
async function performBaselineChot19May() {
  try {
    const filePath = '/home/kata/Coding/rausachfinal/doisoat/Ton-Huy 19-5.xlsx';
    ...
    const tomorrowStart = new Date('2026-05-20T00:00:00+07:00');
    const baselineTime = new Date('2026-05-19T23:59:59+07:00');
    ...
    console.log(`(Orders with ngaygiao >= 2026-05-20 will be preserved as 'dadat')`);

    // 3. Create Master Chotkho
    const chotkhoMaster = await prisma.chotkho.create({
      data: {
        ngaychot: baselineTime,
        title: `Chốt kho Base Line 19-05-2026`,
        ghichu: 'Chốt kho Baseline theo file Ton-Huy 19-5.xlsx - Không ảnh hưởng đơn ngày 20-05',
        khoId: KHO_TONG_ID,
        codeId: `CHOTKHO_BASELINE_1905_${Date.now()}`,
        isActive: true
      }
    });
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // E. Individual update for Excel products (161 products)
    ...
    // Write report file
    const fs = require('fs');
    const reportPath = `/home/kata/Coding/rausachfinal/report/REPORT_CHOT_BASELINE_19052026.md`;
    const reportContent = `# Báo Cáo Chốt Baseline Tồn Kho - Ngày 19/05/2026
...
| **Đơn hàng chuyển về 'choxuly'** | **\${backloggedDonhang.length + backloggedDathang.length}** | Đơn hàng tồn đọng/quá hạn tính đến hết ngày 19/05 |
