...
async function main() {
  ...
  // Sales Orders (Donhang)
  let dhTotalRevenue = 0;
  let dhStatusCount: Record<string, number> = {};
  donhang.forEach(d => {
  ...
  // Purchase Orders (Dathang)
  let dathangStatusCount: Record<string, number> = {};
  ...
  dathang.forEach(d => {
    ...
    d.sanpham.forEach(sp => {
      ...
      if (Number(sp.gianhap) === 0) zeroPriceCount++;
    });
