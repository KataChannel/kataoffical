...
async function performBaselineChot21May() {
  try {
    const filePath = '/home/kata/Coding/rausachfinal/doisoat/Ton-Huy 21-5.xlsx';
    ...
    // Tomorrow is 2026-05-22
    const tomorrowStart = new Date('2026-05-22T00:00:00+07:00');
    const baselineTime = new Date('2026-05-21T23:59:59+07:00');
    ...
    console.log(`(Orders with ngaygiao >= 2026-05-22 will be preserved as 'dadat')`);

    // 3. Create Master Chotkho
    const chotkhoMaster = await prisma.chotkho.create({
      data: {
        ngaychot: baselineTime,
        title: `Chốt kho Base Line 21-05-2026`,
        ghichu: 'Chốt kho Baseline theo file Ton-Huy 21-5.xlsx - Không ảnh hưởng đơn ngày 22-05',
        khoId: KHO_TONG_ID,
        codeId: `CHOTKHO_BASELINE_2105_${Date.now()}`,
        isActive: true
      }
    });
    ...
    // Write report file
    const fs = require('fs');
    const reportPath = `/home/kata/Coding/rausachfinal/report/REPORT_CHOT_BASELINE_21052026.md`;
    const reportContent = `# Báo Cáo Chốt Baseline Tồn Kho - Ngày 21/05/2026
...
| **Đơn hàng chuyển về 'choxuly'** | **${backloggedDonhang.length + backloggedDathang.length}** | Đơn hàng tồn đọng/quá hạn tính đến hết ngày 21/05 |
