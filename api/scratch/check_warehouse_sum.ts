...
async function main() {
  ...
  for (const sp of products) {
    ...
    // If the logic is that KHO TỔNG = Sum of all virtual warehouses, then:
    // This is one interpretation of "Mirror Logic".
    // However, if some stock is ONLY in KHO TỔNG (not in any virtual warehouse), then KHO TỔNG >= Sum Others.
