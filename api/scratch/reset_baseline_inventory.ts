...
async function run() {
  ...
  ws.eachRow((row, rowNumber) => {
    ...
    // Column 2 (B) is masp, Column 5 (E) is slton based on headers
    // Actually let's be safe and check the object structure
  ...
  for (const sp of allProducts) {
    ...
    const targetQty = new Prisma.Decimal(excelStock !== undefined ? excelStock : 0);
