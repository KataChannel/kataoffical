...
async function performBaselineChot29May() {
  const prisma = new PrismaClient({ datasources: { postgres: { url: DB_URL } } });

  try {
    const filePath = '/home/kata/Coding/rausachfinal/doisoat/Ton-Huy 29-5.xlsx';
    console.log(`--- Processing Baseline & Cleanup (29-05-2026) on rausachfinal DB: ${filePath} ---`);
    ...
    // Tomorrow is 2026-05-30
    const tomorrowStart = new Date('2026-05-30T00:00:00+07:00');
    const baselineTime = new Date('2026-05-29T23:59:59+07:00');
    ...
    // 3. Find old chotkho and vouchers dynamically to clean up
    const oldChotkhos = await prisma.chotkho.findMany({
      where: {
        ngaychot: {
          gte: new Date('2026-05-29T00:00:00+07:00'),
          lte: new Date('2026-05-29T23:59:59+07:00')
        }
      }
    });
    const oldChotkhoIds = oldChotkhos.map(c => c.id);
    console.log(`Found ${oldChotkhoIds.length} old chotkho record(s) to delete:`, oldChotkhoIds);

    const oldVouchers = await prisma.phieuKho.findMany({
      where: {
        maphieu: {
          contains: 'DOISOAT-29052026'
        }
      }
    });
    ...
    const newChotkhoCodeId = `CHOTKHO_BASELINE_2905_${Date.now()}`;
    ...
    // 4. TRANSACTION: Perform all database cleanup and updates atomics
    await prisma.$transaction(async (tx) => {
      ...
      const chotkhoMaster = await tx.chotkho.create({
        data: {
          ...
          title: `Chốt kho Base Line 29-05-2026`,
          ghichu: 'Chốt kho Baseline chuẩn theo file Ton-Huy 29-5.xlsx - Khắc phục lệch giờ các đơn nhập tự động phát sinh sau chốt',
          khoId: KHO_TONG_ID,
      ...
      allProducts.forEach(sp => {
        ...
        chotDetailsData.push({
          ...
          ghichu: hasExcel ? 'Cập nhật chuẩn từ Excel kiểm kho' : 'Tự động reset về 0 (không có trong Excel kiểm kho)'
        });
      });
    ...
    console.log('\n--- BASELINE 29-05 RECONCILIATION COMPLETE ---');
    console.log(`- Old Chotkhos deleted.`);
    console.log(`- Old faulty vouchers deleted.`);
    ...
    console.log(`- Future pending quantities recalculated correctly.`);
