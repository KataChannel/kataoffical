...
async function main() {
    ...
    const cutoff = new Date('2026-05-24T17:00:00+07:00');

    console.log(`Starting Full Baseline Process for 24/05/2026 at Cut-off: ${cutoff.toLocaleString('vi-VN')}`);
    ...
    await prisma.$transaction(async (tx) => {
        ...
        chotkhoMasterCode = `BASELINE_FULL_24052026_${Date.now()}`;
        const chotkhoMaster = await tx.chotkho.create({
            data: {
                ngaychot: cutoff,
                title: `Chốt kho Base Line 24-05-2026`,
                ghichu: 'Reset tồn kho về 0 cho sản phẩm không có trong file. Cập nhật theo thực tế kiểm kê 24-5.',
                khoId: khoId,
        ...
        const detailRecords = details.map(d => {
            ...
            return {
                ...
                ngaychot: cutoff,
                ghichu: excelMap.has(d.masp) ? 'Chốt kho Baseline từ file Ton-Huy 24-5.xlsx' : 'Tự động reset về 0 (không có trong file đối soát)'
            };
        });
    ...
    // 7. Write Report File
    const reportPath = '/home/kata/Coding/rausachfinal/report/REPORT_CHOT_BASELINE_24052026.md';
    const reportContent = `# Báo Cáo Chốt Baseline Tồn Kho - Ngày 24/05/2026
...
- **Thời điểm cut-off:** ${cutoff.toLocaleString('vi-VN')} (Giao dịch trước 17h cùng ngày)
...
| **Đơn hàng chuyển về 'choxuly' (Đơn hàng)** | **${dhCount}** | Các đơn hàng tồn đọng có trạng thái \`dadat\` hoặc \`dagiao\` trước 17:00 ngày 24/05 |
