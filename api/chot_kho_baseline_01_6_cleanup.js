...
async function performBaselineChot01June() {
  const prisma = new PrismaClient({ datasources: { postgres: { url: DB_URL } } });

  try {
    const filePath = '/home/kata/Coding/rausachfinal/doisoat/Ton-Huy 01-6.xlsx';
    console.log(`--- Processing Baseline & Cleanup (01-06-2026) on rausachfinal DB: ${filePath} ---`);
    ...
    // Tomorrow is 2026-06-02
    const tomorrowStart = new Date('2026-06-02T00:00:00+07:00');
    ...
    const oldChotkhos = await prisma.chotkho.findMany({
      where: {
        ngaychot: {
          ...
          lte: new Date('2026-06-01T23:59:59+07:00')
        }
      }
    });
    ...
    const newChotkhoCodeId = `CHOTKHO_BASELINE_0106_${Date.now()}`;
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const chotkhoMaster = await tx.chotkho.create({
        data: {
          ...
          title: `Chốt kho Base Line 01-06-2026`,
          ghichu: 'Chốt kho Baseline chuẩn theo file Ton-Huy 01-6.xlsx - Khắc phục lệch giờ các đơn nhập tự động phát sinh sau chốt',
          khoId: KHO_TONG_ID,
    ...
    console.log('\n--- BASELINE 01-06 RECONCILIATION COMPLETE ---');
