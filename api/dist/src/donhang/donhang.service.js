...
let DonhangService = class DonhangService {
    ...
    async createSummaryExcelFile(groupedData, sortedMonths, params) {
    ...
    async createCongnoExcelFile(data, params) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                worksheet.getRow(1).height = 60 * 0.75;
            }
            else {
                worksheet.mergeCells('A1:P1');
                setFallbackLogo();
    ...
    deduplicateAndMergeSanpham(sanpham) {
        if (!Array.isArray(sanpham))
            return [];
        const merged = [];
        for (const sp of sanpham) {
            ...
            const existing = merged.find(item => (item.idSP || item.id) === idSP &&
    ...
    async create(dto) {
        ...
        const result = await this.prisma.$transaction(async (prisma) => {
            ...
            const resolvedSanpham = sanphamArray.map((sp) => {
    ...
    async _updateInternal(id, data, prisma) {
        ...
        if (isStatusChanged && ['huy', 'choxuly', 'khonggiao', 'dadat'].includes(targetStatus)) {
            ...
            for (const p of productsRaw) {
                ...
                if (productsMap.has(sanphamId)) {
                    ...
                    if (p.ghichu)
                        existing.ghichu = (existing.ghichu ? existing.ghichu + ' | ' : '') + p.ghichu;
                }
                else {
        ...
        const updatedDonhang = await prisma.donhang.update({
            ...
            data: {
                ...
                sanpham: data.sanpham ? {
                    deleteMany: {},
                    create: this.deduplicateAndMergeSanpham(data.sanpham).map((sp) => ({
                        idSP: sp.id || sp.idSP,
        ...
        const { tongvat, tongtien } = this.calculateDonhangTotals(updatedDonhang.sanpham.map(sp => {
    ...
    async verifyOrderPrices(donhangId) {
        try {
            ...
            for (const item of donhang.sanpham) {
                ...
                try {
                    const currentPrice = await this.priceHistoryService.getCurrentPrice(metadata.banggiaId, item.idSP);
                    if (!currentPrice) {
                        discrepancies.push({
                            sanphamId: item.idSP,
                            sanphamCode: item.sanpham.masp,
            ...
            throw new common_1.InternalServerErrorException(error.message || 'Error verifying order prices');
        }
    }
    async getPriceAtDate(banggiaId, sanphamId, date) {
        try {
            ...
            throw new common_1.InternalServerErrorException(error.message || 'Error getting price at date');
        }
    }
    async updateProductPrice(dto) {
        try {
            const donhang = await this.prisma.donhang.findUnique({
            ...
            if (donhang.status === 'dagiao' || donhang.status === 'hoanthanh') {
                throw new common_1.BadRequestException('Không thể sửa giá đơn hàng đã giao/hoàn thành');
            }
            const donhangsanpham = donhang.sanpham[0];
            if (!donhangsanpham) {
                throw new common_1.NotFoundException('Sản phẩm không tồn tại trong đơn hàng');
            }
            ...
            if (Math.abs(changePercent) > 20 && !dto.changeReason) {
                throw new common_1.BadRequestException('Thay đổi giá > 20% yêu cầu nhập lý do');
            }
            ...
            throw new common_1.InternalServerErrorException(error.message || 'Lỗi khi cập nhật giá sản phẩm');
        }
    }
    async getDonhangPriceAudit(donhangId) {
        try {
            ...
            throw new common_1.InternalServerErrorException(error.message || 'Lỗi khi lấy lịch sử giá');
        }
    }
};
