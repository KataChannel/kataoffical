...
async function exportSieuThiExcel(targetDateStr, outputFilePath) {
    ...
    // 2. Fetch Demand / Supply Data (Simplified from NhuCauDatHangResolver)
    ...
    // Map data for Sheet 1
    const tkMap = new Map(tonkhos.map(t => [t.sanphamId, t]));
    const ckMap = new Map(chotkhos.map(c => [c.sanphamId, c]));
    const skMap = new Map();
...
async function main() {
    ...
    const outputFile = path.join(outputDir, `VanDon_SieuThi_17042026_Premium.xlsx`);
