...
async function main() {
    ...
    const cutoff = new Date('2026-05-22T17:00:00+07:00');

    console.log(`Starting Full Baseline Process for 22/05/2026 at Cut-off: ${cutoff.toLocaleString('vi-VN')}`);
    ...
    await prisma.$transaction(async (tx) => {
        ...
        const chotkhoMaster = await tx.chotkho.create({
            data: {
                ngaychot: cutoff,
                title: `Chốt kho Base Line 22-05-2026`,
                ghichu: 'Reset tồn kho về 0 cho sản phẩm không có trong file. Cập nhật theo thực tế kiểm kê 22-5.',
                khoId: khoId,
                codeId: `BASELINE_FULL_22052026_${Date.now()}`,
                isActive: true,
                isLocked: true,
                lockedAt: new Date()
            }
        ...
        const detailRecords = details.map(d => {
            ...
            return {
                ...
                ngaychot: cutoff,
                ghichu: excelMap.has(d.masp) ? 'Chốt kho Baseline từ file Ton-Huy 22-5.xlsx' : 'Tự động reset về 0 (không có trong file đối soát)'
            };
        });
