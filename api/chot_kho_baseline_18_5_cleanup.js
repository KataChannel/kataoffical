...
async function performBaselineChot18May() {
  try {
    const filePath = '/home/kata/Coding/rausachfinal/doisoat/Ton-Huy 18-5.xlsx';
    ...
    // Tomorrow is 2026-05-19
    const tomorrowStart = new Date('2026-05-19T00:00:00+07:00');
    const baselineTime = new Date('2026-05-18T23:59:59+07:00');
    ...
    console.log(`(Orders with ngaygiao >= 2026-05-19 will be preserved as 'dadat')`);

    // 3. Create Master Chotkho
    const chotkhoMaster = await prisma.chotkho.create({
      data: {
        ngaychot: baselineTime,
        title: `Chốt kho Base Line 18-05-2026`,
        ghichu: 'Chốt kho Baseline theo file Ton-Huy 18-5.xlsx - Không ảnh hưởng đơn ngày 19-05',
        khoId: KHO_TONG_ID,
        codeId: `CHOTKHO_BASELINE_1805_${Date.now()}`,
        isActive: true
      }
    });
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // E. Individual update for Excel products (166 products)
    ...
    // Write report file
    const fs = require('fs');
    const reportPath = `/home/kata/Coding/rausachfinal/report/REPORT_CHOT_BASELINE_18052026.md`;
    const reportContent = `# Báo Cáo Chốt Baseline Tồn Kho - Ngày 18/05/2026

## 1. Thông Tin Chung
- **Thời gian thực hiện:** \${new Date().toLocaleString('vi-VN')} (Giờ hệ thống)
- **File nguồn:** \`doisoat/Ton-Huy 18-5.xlsx\`
- **Kho thực hiện:** KHO - HCM (ID: \`4cc01811-61f5-4bdc-83de-a493764e9258\`)
- **Mục tiêu:** Thiết lập lại Baseline cho ngày 18/05/2026.
...
| **Đơn hàng chuyển về 'choxuly'** | **\${backloggedDonhang.length + backloggedDathang.length}** | Đơn hàng tồn đọng/quá hạn tính đến hết ngày 18/05 |
...
- **Các bảng dữ liệu đã cập nhật:**
    1.  \`Chotkho\`: Tạo phiên chốt mới cho ngày 18/05.
    2.  \`Chotkhodetail\`: Lưu chi tiết chênh lệch cho các sản phẩm.
    3.  \`SanphamKho\`: Cập nhật số lượng tồn thực tế.
    4.  \`TonKho\`: Cập nhật \`slton\`, \`sltontt\` và tính toán lại \`slchogiao\`, \`slchonhap\` dựa trên các đơn hàng tương lai (ngày 19/05 trở đi).

## 4. Ghi Chú Vận Hành
- Phiên chốt này đã giải quyết triệt để các vấn đề "Ghost Reservations" (tồn ảo dự phòng) và các đơn hàng cũ quá hạn.
- Hệ thống hiện đã ở trạng thái Baseline sạch cho ngày mới 19/05/2026.

---
*Báo cáo được thực hiện tự động bởi Antigravity AI - 18/05/2026*
`;
    // We need to resolve the dynamic interpolation in reportContent
    const populatedReport = reportContent
      .replace('${new Date().toLocaleString(\'vi-VN\')}', new Date().toLocaleString('vi-VN'))
      .replace('${totalProducts}', totalProducts)
      .replace('${excelProductsCount}', excelProducts.length)
      .replace('${resetProductsCount}', resetProducts.length)
      .replace('${backloggedDonhang.length + backloggedDathang.length}', backloggedDonhang.length + backloggedDathang.length)
      .replace('${chotkhoMaster.id}', chotkhoMaster.id)
      .replace('${chotkhoMaster.codeId}', chotkhoMaster.codeId);

    fs.writeFileSync(reportPath, populatedReport, 'utf8');
