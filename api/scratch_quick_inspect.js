...
async function main() {
  try {
    ...
    if (latestChotkho) {
      ...
      // Let's list recent chotkho
      const recentChotkhos = await prisma.chotkho.findMany({
        take: 5,
        orderBy: { ngaychot: 'desc' }
      });
      console.log("Recent Chotkhos in DB:");
      recentChotkhos.forEach(c => console.log(`- ${c.ngaychot} | ${c.title} | ${c.id}`));
    }
