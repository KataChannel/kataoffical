...
export class GiohangcommonComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.ColumnName = Object.fromEntries(
      Object.entries(this.ColumnName).filter(
        ([key]) => !this.HideColumns.includes(key)
      )
    );
    ...
    this.FilterSanphamsBansi = this.Sanphams.filter(
      (v) => !this.Donhang.Giohangs.some((v1: any) => v1.MaSP === v.MaSP)
    ).map((v: any) => ({
      id: v.id,
      MaSP: v.MaSP,
      giagoc: Number(v.giagoc),
  ...
  onValueChange(value: any, idx: any, fieldSL: any, fieldTong: any) {
    ...
    if (valueNumber <= 0) {
      ...
      this.Donhang.Giohangs[idx][fieldSL] = valueNumber;
    }

    switch (fieldSL) {
      case 'Soluong':
        this.Donhang.Giohangs[idx].SLTG = valueNumber;
        this.Donhang.Giohangs[idx].Tongtien = this.Donhang.Giohangs[
          idx
        ].TongtienG = valueNumber * this.Donhang.Giohangs[idx].giagoc;
        break;
      case 'SLTG':
        this.Donhang.Giohangs[idx].SLTN = valueNumber;
        this.Donhang.Giohangs[idx].TongtienG = this.Donhang.Giohangs[
          idx
        ].TongtienN = valueNumber * this.Donhang.Giohangs[idx].giagoc;
        break;
      default:
        this.Donhang.Giohangs[idx][fieldTong] =
          valueNumber * this.Donhang.Giohangs[idx].giagoc;
        break;
    }
  ...
  ChangeSoluong(idx: any, method: any, fieldTong: any, fieldSL: any) {
    switch (fieldSL) {
      case 'Soluong':
        this.Donhang.Giohangs[idx].Soluong =
          Number(this.Donhang.Giohangs[idx][fieldSL]) || 0;
        if (this.Donhang.Giohangs[idx][fieldSL] <= 0 && method === 'giam') {
          ...
          const adjustment = method === 'giam' ? -1 : 1;
          this.Donhang.Giohangs[idx].Soluong += adjustment;
          this.Donhang.Giohangs[idx].SLTG = this.Donhang.Giohangs[idx].SLTN =
            this.Donhang.Giohangs[idx].Soluong;
          this.Donhang.Giohangs[idx].Tongtien =
            this.Donhang.Giohangs[idx].TongtienG =
            this.Donhang.Giohangs[idx].TongtienN =
              this.Donhang.Giohangs[idx].Soluong *
              this.Donhang.Giohangs[idx].giagoc;
        }
        break;
      case 'SLTG':
        this.Donhang.Giohangs[idx].SLTG =
          Number(this.Donhang.Giohangs[idx][fieldSL]) || 0;
        if (this.Donhang.Giohangs[idx][fieldSL] <= 0 && method === 'giam') {
          this._snackBar.open('Giá trị phải lớn hơn 0', '', {
            duration: 1000,
          ...
          const adjustment = method === 'giam' ? -1 : 1;
          this.Donhang.Giohangs[idx].SLTG += adjustment;
          this.Donhang.Giohangs[idx].SLTN = this.Donhang.Giohangs[idx].SLTG;
          this.Donhang.Giohangs[idx].TongtienG = this.Donhang.Giohangs[
            idx
          ].TongtienN =
            this.Donhang.Giohangs[idx].SLTG *
      ...
      default:
        ...
        if (this.Donhang.Giohangs[idx][fieldSL] <= 0 && method === 'giam') {
          ...
          const adjustment = method === 'giam' ? -1 : 1;
          this.Donhang.Giohangs[idx][fieldSL] += adjustment;
          this.Donhang.Giohangs[idx][fieldTong] =
            this.Donhang.Giohangs[idx][fieldSL] *
            this.Donhang.Giohangs[idx].giagoc;
        }
  ...
  Chonsanpham(data: any, giagoc: any) {
    ...
    if (item.MaSP == '-1') {
      ...
      const existingItemIndex = this.Donhang.Giohangs?.findIndex(
        (v: any) => v.MaSP === data.MaSP
      );
      if (existingItemIndex !== -1) {
        this.Donhang.Giohangs[existingItemIndex].Soluong += Number(
          item.Soluong
        );
        ...
        item.Tongtien = item.SLTT * item.giagoc;
        item.SLTG = 0;
        item.TongtienG = 0;
        item.SLTN = 0;
        item.TongtienN = 0;
  ...
  Xoasanpham(item: any) {
    console.log(item);
    this.Donhang.Giohangs = this.Donhang.Giohangs.filter(
      (v: any) => v.MaSP != item.MaSP
    );
    this.dataSource = new MatTableDataSource(this.Donhang.Giohangs);
    this.GiohangsEmit.emit(this.Donhang.Giohangs);
  }
  ...
  TinhTong(items: any, fieldTong: any) {
    const subtotal =
      items?.reduce(
        (sum: any, item: any) => sum + (item['Tongtien'] || 0),
        0
      ) || 0;
  ...
  writeExcelFile() {
    ...
    this.saveAsExcelFile(
      excelBuffer,
      'giohang_' + moment().format('DD_MM_YYYY')
    );
  }
