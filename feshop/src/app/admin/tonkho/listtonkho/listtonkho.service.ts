...
export class TonkhosService {
  ...
  async getAllTonkho() {
    try {
      ...
      const response = await fetch(`${environment.APIURL}/tonkhos`, options);
      if (!response.ok) {
        if (response.status === 401) {
          const result  = JSON.stringify({ code:response.status,title:'Vui lòng đăng nhập lại' })
          this.router.navigate(['/errorserver'], { queryParams: {data:result}});
  ...
  async SearchTonkho(SearchParams:any) {
    try {
      const options = {
        method:'POST',
        headers: {
  ...
  async getTonkhoByid(id: any) {
    try {
      ...
      const response = await fetch(`${environment.APIURL}/tonkhos/${id}`, options);
      if (!response.ok) {
        if (response.status === 401) {
          const result  = JSON.stringify({ code:response.status,title:'Vui lòng đăng nhập lại' })
          this.router.navigate(['/errorserver'], { queryParams: {data:result}});
      ...
      const data = await response.json();
      this.Tonkho.set(data)
    } catch (error) {
      return console.error(error);
    }
  }
  async updateOneTonkho(dulieu: any) {
    try {
      const options = {
          method:'PATCH',
          headers: {
        ...
        const response = await fetch(`${environment.APIURL}/tonkhos/${dulieu.id}`, options);
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
  ...
  async DeleteTonkho(item:any) {    
    try {
        const options = {
          ...
          const response = await fetch(`${environment.APIURL}/tonkhos/${item.id}`, options);
          if (!response.ok) {
            if (response.status === 401) {
              const result  = JSON.stringify({ code:response.status,title:'Vui lòng đăng nhập lại' })
              this.router.navigate(['/errorserver'], { queryParams: {data:result}});
