...
let UsersService = class UsersService {
    constructor(prisma, telegramService, tonService) {
        this.prisma = prisma;
    ...
    async plantCrop(userId, cropName) {
        ...
        if (this.crops[cropName].premium)
            user.balance -= 0.5;
    ...
    async waterCrops(userId) {
        const user = await this.getUser(userId);
        const currentTime = Date.now() / 1000;
        if (!user.crops.length)
            throw new Error('No crops to water');
    ...
    async harvestCrops(userId) {
        ...
        if (!matureCrops.length)
            throw new Error('No mature crops');
        const results = [];
        let totalTon = 0;
        const fee = 0.005 * matureCrops.length;
        if (user.balance < fee)
            throw new Error(`Need ${fee} TON testnet`);
        user.balance -= fee;
        ...
        if (user.wallet && totalTon > 0) {
            try {
                ...
                await this.tonService.sendTransaction(masterWallet, user.wallet, totalTon.toString(), `Harvest ${matureCrops.length} crops (Testnet)`);
                user.balance += totalTon;
                tokenMessage = `Received ${totalTon} TON testnet`;
                if (user.telegramChatId) {
                    await this.telegramService.sendNotification(user.telegramChatId, `Bạn vừa thu hoạch ${matureCrops.length} cây! Nhận ${user.xp} XP và ${totalTon} TON testnet.`);
                }
            }
            catch (error) {
    ...
    async withdraw(userId, amount) {
        ...
        try {
            const masterWallet = await this.tonService.initializeWallet(process.env.MASTER_WALLET_SEED.split(' '));
            await this.tonService.sendTransaction(masterWallet, user.wallet, amountAfterFee.toString(), `Withdraw from Ngu Hanh Farm (Testnet)`);
            user.balance -= amount;
            await this.prisma.user.update({ where: { userId }, data: { balance: user.balance } });
            if (user.telegramChatId) {
    ...
    async linkTelegram(userId, telegramChatId) {
        const user = await this.getUser(userId);
